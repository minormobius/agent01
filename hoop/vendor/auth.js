/**
 * ⚠ VENDORED — verbatim copy of /packages/oauth-client/auth.js.
 *
 * hoop/ is a no-build static site served by a Cloudflare ASSETS binding, so a
 * browser `import` can only reach files inside hoop/. Rather than hand-roll an
 * auth client (which CLAUDE.md explicitly warns against — photo/wave/wiki each
 * drifted), we keep an EXACT copy here. If the shared lib changes, re-copy it:
 *   cp packages/oauth-client/auth.js hoop/vendor/auth.js   (then re-add this banner)
 * Do not edit the logic below; the only intentional diff from source is this note.
 */
