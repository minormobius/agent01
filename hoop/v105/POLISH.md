...
- [ ] records page (`/v105/records`) — note that the verdict feed is now consumed live.
...
- [x] **multi-profile saves + world loader** — each ship seed is a separate save (PDS record keyed by
      seed + a per-world local buffer, `hoop:v105:story:<seed>`, migrated off the legacy global). Boot
      resumes the last-played world (`hoop:v105:lastseed` / `?seed=N`). The ⟲ "worlds" dock button opens
