...
export function buildMystery(content, cast, worldSeed, opts = {}) {
  ...
  // the case-giver: the LAST ward gate's cast keeper (the final keeper Solen tasks you with finding). The
  // case must surface only at the END of Solen's tier — meeting a keeper sets their own gate, and the
  // case opens on `requires: { [caseGiver.gate]: true }`, so keying it to the last-met keeper keeps the
  // investigation off the FIRST keeper (v107 fix: the first Factor Solen keeper was offering the murder).
  const entries = anchor.gates.map((g) => cast.byGate[g]).filter((e) => e && e.keeperId && !e.briefing);
  const cgEntry = entries[entries.length - 1];
