# v099 — the home stretch (polish punch-list)

v099 = v097 + the rumor-mill verdict feed wired into the live load path. The big systems are in
...
- [ ] records page (`/v099/records`) — note that the verdict feed is now consumed live.
...
- [x] **multi-profile saves + world loader** — each ship seed is a separate save (PDS record keyed by
      seed + a per-world local buffer, `hoop:v099:story:<seed>`, migrated off the legacy global). Boot
      resumes the last-played world (`hoop:v099:lastseed` / `?seed=N`). The ⟲ "worlds" dock button opens
...
- [x] fixed two pre-existing bugs surfaced by the audit: `#arch` was unpositioned + unclickable
      (`.hud` is `pointer-events:none`); `#newgame` overlapped `#pack` at bottom-right. Title v096→v099.
