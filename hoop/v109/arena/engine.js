...
// ── PLANET RPS (v105 unified language) — "element over element." A unit carries a PLANET (its character's
...
export function makeUnit({ id, name, team, faction, character, combat, x, y, sprite, glyph, accent, summoned = false, ai = null, kit = null, mods = null, body = 'solid', footprint = 1, planet = null }) {
  ...
  return {
    ...
    planet: planet || (character && character.planet) || null,   // v105: the unit's register — drives the element RPS
...
function resolveAttack(s, atk, tgt, skillId, isCounter = false) {
  ...
  const elem = elementMult(atk.planet, tgt.planet);                 // v105 element-over-element (×1 if either is planet-less)
