...
//   • the gate's setting choice is REACHABLE inside the setter's dialogue (v104): walking from `start`
...
// to embiggen is guaranteed by flagKeeperResident's low-vitality fallback — and (v104, the Miren
...
export function proveProgression(content, opts = {}) {
  ...
  for (const a of chain) {
    ...
    for (const g of a.gates) {
      ...
      // the flag-setting choice must be REACHABLE inside the setter's own dialogue (v104): from `start`,
...
// v104 FUNGIBLE: the active anchor's unmet gates, EACH with the full list of its setter content ids. A gate is
// satisfied by ANY of these keepers, so the surface guarantees ≥1 of each list is PLACED (and points the ◇ at
// the nearest placed one) rather than force-placing one hardcoded keeper. `settersMulti` = gateSettersMulti().
export function requiredGateSetters(chain, settersMulti, facts, tier) {
  const st = advanceState(chain, facts, tier);
  if (!st || st.allGatesSet) return [];
  const out = [];
  for (const g of st.unmetGates) {
    const ids = (settersMulti[g] || []).map((s) => s.contentId).filter(Boolean);
    if (ids.length) out.push({ gate: g, setterIds: [...new Set(ids)] });
  }
  return out;
}
...
export default { proveProgression, requiredKeeperIds, requiredGateSetters, waivableGates, canReachFlag, ZONE_TIER };
