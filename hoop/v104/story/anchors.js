...
// ── FUNGIBLE gates (v104) ─────────────────────────────────────────────────────────────────────────────
// content[] → { gateFlag: [{contentId, name, room, zone, verb, navefac}, …] } — EVERY setter of each gate,
// not just the first. A gate can now be satisfied by ANY of several keepers (hoopy's diversity model: "ten
// room bundles per chunk, ≥3 fulfill the conditions"), and the surface points the waypoint at the nearest
// PLACED one — the same fungible logic the side-thread `seekCandidates` uses. Deduped by contentId, sorted by
// id (determinism). `gateSetters` (first-only) stays for the solvability oracle, which only needs one.
export function gateSettersMulti(content) {
  const map = {};
  for (const c of content || []) {
    for (const { choice } of eachChoice(c)) {
      const sf = (choice.effects && choice.effects.set_facts) || {};
      for (const k of Object.keys(sf)) {
        if (!GATE_RE.test(k) || CLEAR_RE.test(k) || isChoiceFlag(k)) continue;
        (map[k] || (map[k] = [])).push({
          contentId: c.id, name: npcName(c),
          room: c.roomName || (c.content && c.content.name) || null,
          zone: (c.content && c.content.zone) || c.zone || null,
  ...
  for (const k of Object.keys(map)) {
    const seen = new Set();
    map[k] = map[k].filter((s) => (seen.has(s.contentId) ? false : seen.add(s.contentId)))
      .sort((a, b) => (a.contentId < b.contentId ? -1 : 1));
  }
  return map;
}
...
//
// v104 (the Factor Solen bug): an anchor like Solen spans the "wards" zone — its gates name keepers in the
...
//
// v104 (FUNGIBLE): `setters` may be the first-only map (gateSetters) OR the multi map (gateSettersMulti). The
// returned object carries `setters` — the FULL list of that gate's satisfiers (so the surface can point the ◇
// at the nearest PLACED one) — plus the first satisfier's fields at the top level (back-compat). Reachability
// is evaluated per-SATISFIER: a gate is reachable if ANY of its setters is, and the returned list is filtered
// to the reachable ones.
export function nextKeeper(chain, setters, facts, tier, opts = {}) {
  ...
  const reach = typeof opts.reachable === 'function' ? opts.reachable : null;
  let fallback = null;
  for (const g of st.unmetGates) {
    const raw = setters[g]; if (!raw) continue;
    const list = Array.isArray(raw) ? raw : [raw];
    if (!list.length) continue;
    if (!fallback) fallback = { flag: g, setters: list, ...list[0] };
    if (!reach) return { flag: g, setters: list, ...list[0] };
    const reachable = list.filter((s) => reach(s));
    if (reachable.length) return { flag: g, setters: reachable, ...reachable[0] };
  }
  return fallback;   // no gate has a reachable setter → the first unmet with a setter (readout still shows the hint)
}
...
export default {
  anchorChain, anchorForTier, gateSetters, gateSettersMulti, advanceState, nextKeeper,
