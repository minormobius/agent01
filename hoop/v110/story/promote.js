// hoop/v110/story/promote.js — EMERGENCY NPC PROMOTION (the v105 npc reform). Pure, no DOM/LLM/network.
//
...
// ── one keeper per room (the STACKED KEEPERS bug) ─────────────────────────────────────────────────────────
// v107 reserved each pinned ANCHOR's room so a keeper could never be seated on top of a guide. Nothing did
// the same for keeper-vs-keeper: the kept room is picked `pool[hash % pool.length]`, so any two keepers whose
// hashes collide modulo the pool size land in the SAME room. The click dispatch (residentAt, radius
// playerW·2.6) then gives the click to whichever body is nearer and the other keeper cannot be talked to at
// all — fatal when it is a gate-setter, because its flag can never be set and the tier cannot be turned in.
// The UPPER RIND hits this first and hardest: four chunks, few doored rooms each, so the pool is tiny and
// collisions are likely on the very first keeper.
//
// Preference order, widening only as far as it must so a keeper is NEVER left unseated:
//   1. a room neither an anchor nor a keeper holds   — the normal case
//   2. a room only an ANCHOR holds                   — the anchor's pin and the keeper's putter cells differ
//   3. a room only another KEEPER holds              — thin chamber; the surface then applies a putter keepout
//   4. anything in the pool                          — last resort, never fail to seat
// Rooms are opaque tokens and the held-sets accept a Set or an array, so this is node-testable.
export function pickKeptRoom(pool, { anchorHeld = null, keeperHeld = null, hash = 0 } = {}) {
  const rooms = (pool || []).filter((r) => r != null);
  if (!rooms.length) return null;
  const has = (s, r) => !!(s && (typeof s.has === 'function' ? s.has(r) : Array.isArray(s) && s.includes(r)));
  const tier = [
    rooms.filter((r) => !has(anchorHeld, r) && !has(keeperHeld, r)),
    rooms.filter((r) => !has(keeperHeld, r)),
    rooms.filter((r) => !has(anchorHeld, r)),
    rooms,
  ].find((t) => t.length) || rooms;
  const h = Number.isFinite(hash) ? Math.abs(Math.trunc(hash)) : 0;
  return tier[h % tier.length];
}

export default { promotedId, isPromoted, namesAPerson, personRef, emergencyNpc, needsPromotion, keeperSeatChunks, needsWardReseat, pickKeptRoom };
