// node hoop/v110/test/guides.selftest.mjs
...
// IDENTITY, not a substring: `includes('olo')` also matches "Skerry, called the Colophon", which is
// exactly what the 2026-09-16 run's 409 new names did to tier 1. Pin each guide to the load-bearing
// anchor of its tier — the only record that can actually take a turn-in.
const chain = anchorChain(content);
ok(chain.length === 4, 'the export carries the four load-bearing anchors');
for (let i = 0; i < 4; i++) ok(guides[i] && chain[i] && guides[i].id === chain[i].id,
  `tier ${i + 1}'s guide IS the tier-${i + 1} anchor (${chain[i] ? chain[i].name : '—'}), not a name that merely contains it`);
