...
// room programme — a role mix with target RADII (px); civic wells are rarer & bigger. A room is a
// FOOTPRINT (a disc of this radius) that later claims the small unit cells whose centres fall in it,
// so the painted room is an organic union of many cells, not one convex Voronoi polygon.
...
export function genLayout(opts = {}) {
  const { W = 2600, H = 1500, seed = 1, hallSeg = 175, density = 1, margin = 22, roomSize = 1 } = opts;
  ...
  // 3. ROOMS bud off corridor nodes as DISCS (radius by role × roomSize), perpendicular to the local
  //    corridor, non-overlapping & clear of halls. The disc is the footprint; the unit cells inside it
  //    become the organic room at paint time.
  const rooms = [];
  const clearOfHalls = (x, y, r) => {
    for (const [u, v] of edges) if (segDist(x, y, nodes[u].x, nodes[u].y, nodes[v].x, nodes[v].y) < r + margin * 0.6) return false;
    return true;
  };
  for (const id of corridorNodes) {
    ...
    for (const side of [1, -1]) {                             // try BOTH sides of the corridor
      ...
      const prog = pickRole(rnd), radius = prog.radius * roomSize * (0.85 + rnd() * 0.35);
      const off = hallSeg * 0.4 + radius;                     // push the disc off the corridor by a door stub
      const cx = nodes[id].x + px * side * off, cy = nodes[id].y + py * side * off;
      if (cx - radius < margin || cy - radius < margin || cx + radius > W - margin || cy + radius > H - margin) continue;
      if (!clearOfHalls(cx, cy, radius)) continue;
      if (rooms.some((r) => dist(r, { x: cx, y: cy }) < r.radius + radius + margin)) continue;
      // the single DOOR: where the disc meets the corridor (on the room edge facing the node)
      const doorPt = { x: nodes[id].x + px * side * (off - radius), y: nodes[id].y + py * side * (off - radius) };
      rooms.push({ id: rooms.length, x: cx, y: cy, radius, role: prog.role, doorHall: id, doorPt });
    }
  }
...
// point-in-room test (disc), used by nav + the page (which room did I tap?)
export function roomAt(layout, x, y) {
  for (const r of layout.rooms) if ((x - r.x) ** 2 + (y - r.y) ** 2 <= r.radius * r.radius) return r;
  return null;
}
