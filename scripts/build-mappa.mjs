...
#tip b{color:var(--gold)}#tip .m{color:#9c8a6c;font-size:11px}#tip .open{color:var(--gold);font-size:10.5px;letter-spacing:.03em}
@media (max-width:640px){
  ...
  footer{flex-direction:column;align-items:stretch;gap:8px;padding:8px 10px 12px}
  .scrub{max-width:none}
  .legend{max-width:none;justify-content:flex-start;flex-wrap:nowrap;overflow-x:auto;-webkit-overflow-scrolling:touch;padding-bottom:2px}
  .legend::-webkit-scrollbar{display:none}
  .chip{flex:0 0 auto}
}
@media (max-width:640px) and (display-mode:standalone){footer{padding-bottom:calc(12px + env(safe-area-inset-bottom))}}
</style>
</head>
<body>
...
const terr=document.createElement('canvas'); let terrOW=0, terrOH=0;
const GW=200,GH=130;
const hsl=(h,s,l,a)=> 'hsl('+h+' '+s+'% '+l+'%'+(a!=null?' / '+a:'')+')';
const wing=id=>D.wings.find(w=>w.id===id);
function fmtDate(b){return b||'—'}

function build(){ world=makeWorld(seed,GW,GH,D.wings,D.nodes); fit(); renderTerrain(); draw(); }
...
// Rasterize terrain ONCE to an offscreen canvas (depends only on world + active).
// pan/pinch/time/hover then just blit this — cheap enough for mobile.
function renderTerrain(){
  const TS=4, ow=GW*TS, oh=GH*TS; terr.width=ow; terr.height=oh; terrOW=ow; terrOH=oh;
  const tx=terr.getContext('2d'); const {elev,land,country,mtn,seaLevel}=world;
  for(let j=0;j<GH;j++)for(let i=0;i<GW;i++){const idx=j*GW+i;const x=i*TS,y=j*TS;
    let col;
    if(!land[idx]){ const d=Math.max(0,seaLevel-elev[idx]); col=hsl(205,38,Math.max(8,20-d*30)); }
    else { const id=D.wings[country[idx]].id; col=landShade(id,elev[idx],seaLevel);
      if(mtn[idx]) col=hsl(wing(id).hue,12,72);
      if(!active.has(id)) col=hsl(wing(id).hue,8,28); }
    tx.fillStyle=col; tx.fillRect(x,y,TS+1,TS+1);
    if(land[idx]){ const eR=land[idx+1]?elev[idx+1]:elev[idx]; const sh=Math.max(-0.12,Math.min(0.12,(elev[idx]-eR)*1.5));
      if(sh>0.02){tx.fillStyle='rgba(255,245,220,'+sh+')';tx.fillRect(x,y,TS+1,TS+1);}
      else if(sh<-0.02){tx.fillStyle='rgba(0,0,0,'+(-sh)+')';tx.fillRect(x,y,TS+1,TS+1);} }
  }
  // coastline ink
  for(let j=0;j<GH;j++)for(let i=0;i<GW;i++){const idx=j*GW+i; if(!land[idx])continue;
    if((i+1<GW&&!land[idx+1])||(j+1<GH&&!land[idx+GW])||(i>0&&!land[idx-1])||(j>0&&!land[idx-GW])){
      tx.fillStyle='rgba(20,14,8,.5)';tx.fillRect(i*TS,j*TS,TS+1,TS+1);}}
}

function draw(){
  ctx.setTransform(DPR,0,0,DPR,0,0); ctx.clearRect(0,0,W,H);
  ctx.fillStyle='#0c0a07'; ctx.fillRect(0,0,W,H); // ocean void beyond the map
  ctx.save(); ctx.translate(view.x,view.y); ctx.scale(view.s,view.s);
  const cell=world.cell, ox=world.ox, oy=world.oy;
  ctx.imageSmoothingEnabled=true;
  ctx.drawImage(terr,0,0,terrOW,terrOH, ox,oy, GW*cell, GH*cell);
...
function pick(px,py,touch){ const p=screenToWorld(px,py); const cell=world.cell,ox=world.ox,oy=world.oy; let best=null,bd=1e9;
  ...
  return bd< ((touch?22:12)/view.s)**2 ? best : null; }
function clampZoom(s){ return Math.max(0.6,Math.min(8,s)); }
function showTip(c,px,py,touch){ tip.style.opacity=1;
  tip.style.left=Math.max(80,Math.min(innerWidth-80,px))+'px'; tip.style.top=Math.max(54,py)+'px';
  ...
  tip.innerHTML='<b>'+c.n+'</b> '+(c.capital?'★':'')+'<br><span class="m">'+seam+' · founded '+fmtDate(c.b)+'</span>'
    +(c.u?'<br><span class="open">'+(touch?'tap again to open ↗':'click to open ↗')+'</span>':''); }

// Unified pointer handling: 1 finger/mouse = pan, 2 fingers = pinch-zoom, tap = select/open.
const ptrs=new Map(); let gesture=null, tapStart=null;
const dist=(a,b)=>Math.hypot(a.x-b.x,a.y-b.y);
cv.addEventListener('pointerdown',e=>{ cv.setPointerCapture(e.pointerId);
  ptrs.set(e.pointerId,{x:e.clientX,y:e.clientY});
  if(ptrs.size===1){ tapStart={x:e.clientX,y:e.clientY,t:Date.now(),touch:e.pointerType!=='mouse'};
    gesture={mode:'pan',x:e.clientX,y:e.clientY,vx:view.x,vy:view.y}; cv.classList.add('drag'); }
  else if(ptrs.size===2){ const p=[...ptrs.values()];
    gesture={mode:'pinch',d:dist(p[0],p[1]),s:view.s,vx:view.x,vy:view.y}; tapStart=null; tip.style.opacity=0; }
});
cv.addEventListener('pointermove',e=>{
  if(ptrs.has(e.pointerId)) ptrs.set(e.pointerId,{x:e.clientX,y:e.clientY});
  if(gesture&&gesture.mode==='pinch'&&ptrs.size>=2){ const p=[...ptrs.values()];
    const nd=dist(p[0],p[1]); const ns=clampZoom(gesture.s*(nd/(gesture.d||1)));
    const cmx=(p[0].x+p[1].x)/2, cmy=(p[0].y+p[1].y)/2;
    const rx=(cmx-gesture.vx)/gesture.s, ry=(cmy-gesture.vy)/gesture.s; // anchor pinch on the midpoint
  ...
  if(e.pointerType==='mouse'&&ptrs.size===0){ // hover (desktop only)
    const c=pick(e.clientX,e.clientY); if(c!==hot){hot=c;draw();}
    if(c){ showTip(c,e.clientX,e.clientY,false); cv.style.cursor='pointer'; }
    else { if(!selected) tip.style.opacity=0; cv.style.cursor='grab'; } }
});
function endPtr(e){ ptrs.delete(e.pointerId);
  if(ptrs.size===0){ cv.classList.remove('drag');
    if(tapStart && Date.now()-tapStart.t<400){ const c=pick(tapStart.x,tapStart.y,tapStart.touch);
      if(c){ if(!tapStart.touch){ if(c.u) window.open(c.u,'_blank'); } // desktop: single click opens (hover already showed it)
             else if(selected===c && c.u){ window.open(c.u,'_blank'); } // touch: tap to select, tap again to open
             else { selected=c; hot=c; showTip(c,tapStart.x,tapStart.y,true); draw(); } }
      else { selected=null; hot=null; tip.style.opacity=0; draw(); } }
    gesture=null; tapStart=null;
  } else if(ptrs.size===1){ const p=[...ptrs.values()][0]; // pinch → resume one-finger pan
    gesture={mode:'pan',x:p.x,y:p.y,vx:view.x,vy:view.y}; tapStart=null; }
}
cv.addEventListener('pointerup',endPtr);
cv.addEventListener('pointercancel',endPtr);
cv.addEventListener('wheel',e=>{e.preventDefault();const f=e.deltaY<0?1.12:1/1.12;const ns=clampZoom(view.s*f);
...
cv.addEventListener('dblclick',e=>{const ns=clampZoom(view.s*1.8);const rx=(e.clientX-view.x)/view.s,ry=(e.clientY-view.y)/view.s;
  view.x=e.clientX-rx*ns;view.y=e.clientY-ry*ns;view.s=ns;draw();});
...
for(const w of D.wings){const c=document.createElement('span');c.className='chip';c.dataset.w=w.id;
  ...
  c.onclick=()=>{ if(active.size===1&&active.has(w.id))active=new Set(D.wings.map(x=>x.id)); else active=new Set([w.id]);
    for(const ch of lg.children) ch.classList.toggle('off',!active.has(ch.dataset.w)); renderTerrain(); draw(); };
  lg.appendChild(c);}
