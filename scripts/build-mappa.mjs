...
const r = emit(join(root, 'mappa', 'sites.js'), out, { write: !check });
if (check) {
  if (!r.same) { console.error('STALE: mappa/sites.js differs from catalogue.json — run `node scripts/build-mappa.mjs`'); process.exit(1); }
  console.log('mappa/sites.js is current ('+SITES.length+' surfaces)');
  process.exit(0);
}
