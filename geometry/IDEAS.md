...
| `/cohomology/` | **Discrete Hodge decomposition** on a triangulated plane with holes (Hodge 1941; Eckmann's combinatorial version 1944) | Split a random 1-form into exact ⊕ coexact ⊕ harmonic — an orthogonal split into two sparse Laplace solves. The harmonic remainder is pure topology: dim ker Δ₁ = b₁ = number of voids, one independent circulation each, amber counterclockwise and teal clockwise. Tabs: decompose (particle-advected Whitney interpolation, four fields, energy bar), periods (draw cycles by hand along mesh edges; ∮h depends only on the winding vector, ∮dα is identically 0, and the dual basis satisfies ∮ₖhₘ = δₖₘ), ledger (every residual live), poster (art #1148, full-bleed, saveable PNG). Engine is `hodge.js`, shared byte-for-byte with `hodge.selftest.mjs`: 362 checks over 18 mesh configurations — orthogonality, Pythagoras, Δ₁h = 0 at 1e-13, b₁ two independent ways, period invariance, Whitney fidelity. | olive brass `#5f6b16` |
| `/voronoi/` | **Conway's Game of Life on a periodic Voronoi mesh** (Conway 1970, generalised off the grid) | `B3/S23` is a statement about squares, not about life: "three" only means anything because every cell has eight neighbours. On a Voronoi tessellation of a torus cells have 4–8 sides, so the rule is restated as a **fraction** of whatever neighbourhood a cell drew — which specialises back to Conway exactly on a degree-8 Moore grid, and the selftest holds it there cell-for-cell against a naive B3/S23 over gliders, a pulsar and 200 soups × 40 gens. Mesh is built by half-plane clipping with a *proved* stopping rule (a site further than 2r cannot touch the cell) and tagged edges, so adjacency is recorded at the cut rather than reverse-engineered by distance; Lloyd relaxation descends a measured CVT energy. The whole thing is pinned by one number: Σdeg = 6n exactly, forced by V−E+F=0 with trivalent Voronoi vertices, which no dropped or phantom neighbour can fake. Procgen end to end — two seeds generate everything and a permalink reproduces it bit-for-bit — with a hunt that rolls soups until one clears an explicit `emergence` score and shows you every reject. Six specimens found by a 6099-trajectory offline sweep, incl. one unrepeating at 5000 gens, a 1257-generation transient, and a period-210 attractor that is just lcm(2,3,5,6,14). Engine `life.js` shared byte-for-byte with `life.selftest.mjs`: 154 checks. | orchid `#7b2d8b` |
| `/equivelar/` | **Eight faces, all touching** — an equivelar {9,3} polyhedron of genus 3 (Mizhaev, arXiv:2609.17700, Sept 2026) | The sequel to `/szilassi/`'s open question. Eight planar nonagons, 24 corners, 36 edges, three faces per corner, and all **28** face pairs adjacent — the first solid past seven. It beats the count `h = (f−4)(f−3)/12` (whole only for f ≡ 0, 3, 4, 7 mod 12) by breaking its hidden assumption: **eight of the 28 pairs share TWO edges**, the *overarching* case Grünbaum–Szilassi assume away, with both edges on the one line where the two planes cross. So it answers the question as written and leaves f = 12, h = 6 untouched — the page says both. Hero is the 8×8 adjacency grid: every cell filled, eight holding a 2, tap one to light that pair and its shared edge(s). Eight planes are the whole solid (the szilassi trick, one plane up) → **17** shape freedoms vs 14, but only 4.1% clearance, so it is far more fragile: a ±0.25 nudge finds no solids at all. Engine `poly.js` shared byte-for-byte with `poly.selftest.mjs`: 288 checks, the certificate re-derived in **exact integer arithmetic** (every quantity under 2⁵³, asserted, no tolerance) plus a 30 000-point independent resampling of the 28 crossing lines. Two precisions the paper leaves implicit: its orientation signs point the normals inward, and ⟨T⟩ ≅ C₄ is the *abstract* group — det T = −1, so the point group is **S₄**, a rotary reflection, which is why the symmetry lock must flip the tilt sign round each orbit. | indigo `#4146bd` |
| `/chair44/` | **The first strongly aperiodic monotile in 3D** — Chair44 (Tsiokos, arXiv:2609.19214, Sept 2026; found with an AI model, Lean-checked; Flicker arXiv:2609.23783, Goodman-Strauss arXiv:2609.24779) | A seven-cube chair with 192 tiny signed square pyramids that force the chair's 2× substitution and nothing else. WebGL: the tile (pyramids exaggerated, true scale is ×1), the 8-chair cut, 64- and 512-chair patches with a hierarchical explode, and the 44 legal contacts one by one (plus a random illegal one in red). Engine `tile.js` shared with `tile.selftest.mjs` (320 checks): recomputes the paper's whole finite census in integer eighths — 2388 → 44 matching Figure 7 pose for pose, all proper (homochiral), 19 rotations generating all 24, the unique legal cut of 2P up to its 3-fold turn, 697 → 116 → 44 parent contacts = 2 × atlas, 1 decorated symmetry vs 6 bare, closed mesh of volume 7. | teal `#1f6f8b` |
| `/csaszar/` | **The Császár polyhedron** (Császár 1949) — 7 corners with no diagonals, the dual of `/szilassi/` | The mirror of its dual's puzzle. There seven hexagons must stay flat and seven *planes* make that free; here the faces are triangles, so flatness costs nothing and the seven *points* are the whole object — and both land on **14** shape freedoms, as duals must. The content is the classification instead: the 35 signs of "which side of the plane through three corners does the fourth lie on" cannot change while the shape stays a solid (four coplanar corners would put two K₇ edges in a plane, and they would cross), so the realizations sit in **separate connected pieces** and no deformation crosses between them. The *walk* tab walks straight at another one and marks the wall. Four published realizations ship, taken verbatim from Szilassi's own VRML models, differing only in how the three mirror pairs and the lone corner stack up the axis. Not four in total, though — Bokowski–Eggert 1991 count **72**, and `census.mjs` finds 62 of them in 24M random rolls (1 solid in 5,800). Engine `poly.js` shared byte-for-byte with `poly.selftest.mjs`: 168 checks, including a second independent embeddedness oracle and the duality against `szilassi/poly.js`. | leaf green `#4a7c18` |
| `/szilassi/` | **The Szilassi polyhedron** (Szilassi 1977) — 7 hexagons on a torus, every pair sharing an edge | Spinnable WebGL model built for a phone: object in the top two thirds, controls in the bottom third. The hook is that **seven planes are the entire solid** — every corner is 3-valent, so it is where three face planes cross, and nothing is ever solved for. That makes the shape a measured **14-parameter family** (21 plane numbers − 7 similarities; 7 with the half-turn held), exposed as pick-a-face → tilt, tilt, slide. Flatness survives anything; the *solid* does not, and the verdict chip names the two faces that crossed. Acopticity is decided exactly, by walking the line where two face planes meet — the cheap one-sided test is wrong here, and the published solid is the counterexample. Engine `poly.js` shared byte-for-byte with `poly.selftest.mjs`: 449 checks, including Heawood from the shape on screen. Coordinates are Grünbaum–Szilassi 2009 Table 3, planar to the last bit of a double. | magenta `#8b1f7a` |
...
- **`/cohomology/`** — discrete Hodge decomposition on a punctured plane: the cheapest place in the pack where a topological invariant is *computed* rather than argued about. **✓ SHIPPED — see Built table.**
