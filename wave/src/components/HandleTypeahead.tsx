...
export function HandleTypeahead({
  value, onChange, onSelect, placeholder = 'handle.bsky.social', disabled, autoFocus,
}: Props) {
  ...
  const select = useCallback((actor: Actor) => {
  ...
  return (
    <div className="wave-ta-wrap">
      ...
      />
      {open && (
        <div className="wave-ta-drop">
          {actors.map((a, i) => (
            <div
              key={a.did}
              className={`wave-ta-item${i === activeIdx ? ' active' : ''}`}
              onMouseDown={e => { e.preventDefault(); select(a); }}
            >
              {a.avatar ? (
                <img className="wave-ta-av" src={a.avatar} alt="" loading="lazy" />
              ) : (
                <div className="wave-ta-av-ph" />
              )}
              <div className="wave-ta-info">
                {a.displayName && <div className="wave-ta-name">{a.displayName}</div>}
                <div className="wave-ta-handle">@{a.handle}</div>
              </div>
            </div>
          ))}
        </div>
