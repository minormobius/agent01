...
//
// THE PAGE OWNS NO PROSE AT ALL ANY MORE. Every sentence on the campaign panel
// is a campaign.mjs export rendered by reference — the level title and blurb,
// the two game-state lines (lp-3302ee), and the first screen (lp-ef6599). So
// sections 8, 9b and 9c assert WIRING and DEPARTURE and say nothing about what
// the words are like: a check on a string the page owns can only describe it,
// never confirm it is the right one, and those prose rules are asserted once
// each in campaign.selftest.mjs against the exports themselves.
//
