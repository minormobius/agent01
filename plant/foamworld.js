// PORTED FROM foam/foamworld.js — byte-identical at the time of the port.
//
// plant/ is the loop's tree and foam/ is hand-authored; the loop cannot reach
// foam, so a primitive it needs has to arrive here as an explicit copy. See
// CLAUDE.md, "Seeding from foam": this is a COPY and the two trees are meant to
// diverge, so do not treat drift from foam as a bug — porting a foam change
// across is a deliberate act, not an automatic rebase.
//
// WHY THIS ONE. `generatePocket` and `reformPocket` are the only things in the
// repo that know what a real pocket is: seeds, closure, navigation, and the
// indestructible hull. Without them, `plant/solids.mjs` can say what clearance
// a summon NEEDS and nothing can say whether a given pocket HAS it — which is
// why "placement in a real pocket" sat unschedulable while six levels of
// production maths got built on top of nothing.
//
