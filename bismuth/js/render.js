...
export class Renderer {
  ...
  thickness(i) {
    const g = this.growth.genome.oxide, b = this.growth.bricks[i];
  ...
  attachInput() {
    ...
    const down = (x, y, id) => { drag = { x, y, id, moved: false }; this.spinVel = 0; this.elVel = 0; };
    const move = (x, y) => {
      if (!drag) return;
    ...
    cv.addEventListener("pointerdown", (e) => { if (e.isPrimary) { down(e.clientX, e.clientY, e.pointerId); cv.setPointerCapture(e.pointerId); } });
