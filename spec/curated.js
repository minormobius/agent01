...
window.SPEC_CURATED = {
  ...
  families: {
    ...
    wormhole: 'generative', idol: 'generative', table: 'generative', bismuth: 'generative',
    g: 'games', torus: 'games', canvas: 'games', pokemon: 'games',
    ink: 'generative', studio: 'generative',
  ...
  descOverrides: {
    ...
    hose: 'The firehose half of the feed stack, and the other end of b.mino.mobi/feedgen. A feed defined by what it excludes — not politics, not porn, not video, not a reply, not in this bot list — has no query you can ask the AppView for, so this service samples Jetstream from a Durable Object — waking hourly, holding the socket open 20 seconds, running every post through each registered feed\'s filters as it arrives, and paging the matches out of a bounded per-feed ring. Sampling rather than streaming keeps it inside the free tier: the firehose is 38.8 posts a second, and watching all of it costs most of the account\'s Durable Object allowance to observe posts nobody reads. It reads a feed\'s definition off its owner\'s PDS either as a native com.minomobi.feedgen.def record or straight off a SkyFeed-era skyfeedBuilder, so a feed built in the now-unmaintained SkyFeed is rehosted by repointing one field on a record its owner already has — same URL, same likes, same subscribers, and finally a video filter.',
    ...
    read: 'The deep-reading wing: annotated medieval tales (Gawain, Culhwch, Orfeo, the Mabinogi branches…) each with a seven-layer apparatus — parallel text, storybook with AI illustrations, character webs, Propp story graphs, Thompson motif indexes, computed mythographs — plus the Pendragon comparative hub, divination/alchemy readers, and a modernist branch (Joyce\'s Portrait) that swaps every folkloric instrument for one built for modernism and adds a style curve measured off the text.',
    hoop: 'The GAME wing of the O’Neill cylinder pack: “the infinite game” — a deterministic glyph-world adventure over an endless ship engine, every place and save an ATProto record. The current version (v109, carrying the frame-rate pass) is mirrored at the domain root; the engine (v099) and the modelling wings it imports (nave, rind, forge, chunkroller, paint, econ) ride along. Everything older was exfiltrated to hoop-archive in the 2026-07 wrap-up.',
    'hoop-archive': 'The hoop museum: the original first-pass @-room forum game at /, the early engine passes v2–v8, and every frozen version snapshot v090–v108, each still independently served — with frozen same-origin copies of the shared dirs they import. Mainline hoop 301-redirects archived paths here. A museum: serving fixes only, no new work.',
  ...
  healthPaths: {
    ...
    'hose.mino.mobi': '/.well-known/did.json',
