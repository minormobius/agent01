...
window.SPEC_CURATED = {
  ...
  descOverrides: {
    ...
    hose: 'The firehose half of the feed stack, and the other end of b.mino.mobi/feedgen. A feed defined by what it excludes — not politics, not porn, not video, not a reply, not in this bot list — has no query you can ask the AppView for, so this service holds one Jetstream WebSocket open in a Durable Object, runs every post through each registered feed\'s filters as it arrives, and pages the matches out of a bounded per-feed ring buffer. It reads a feed\'s definition off its owner\'s PDS either as a native com.minomobi.feedgen.def record or straight off a SkyFeed-era skyfeedBuilder, so a feed built in the now-unmaintained SkyFeed is rehosted by repointing one field on a record its owner already has — same URL, same likes, same subscribers, and finally a video filter.',
