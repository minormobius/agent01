...
window.SPEC_DATA = {
 ...
 "probe": {
  ...
  "results": {
   ...
   "scores.mino.mobi": {
    "code": 200,
    "path": "/api/scores/top?game=curve"
   },
   "ask.mino.mobi": {
    "code": 200,
    "path": "/"
 ...
 "surfaces": [
  ...
  },
  {
   ...
   "pending": false,
   "type": "backend",
   "branch": "claude/mappa-civ-agent-sim-hghwlt",
   "uses": [],
   "provides": null,
   "serves": null,
   "status": null,
   "note": "Coevolutionary agent-based civilization simulation on a mappa world (worker civ, custom_domain civ.mino.mobi). Headless-first engine in mappa/civ/; CORS-open no-key API /api/civ/run + /api/civ/sweep. Bundles mappa/engine.js. No secrets, no D1.",
   "paths": [
    "civ/**",
    "mappa/civ/**",
    "mappa/engine.js",
  ...
  },
  {
   ...
   "pending": false,
   "type": "frontend",
   "branch": "claude/os-deploy-surface-5hrw5w",
   "uses": [],
   "provides": null,
   "serves": null,
   "status": "MANAGED (frontend) — standalone PDS shell ships and works with no backend (login + ls/cat/find/du/blob/curl/sync/sql, all client-side XRPC + WASM CAR parse + DuckDB). The `container` command is gated OFF unless VITE_CONTAINER_API_URL is set, because os-api (Cloudflare Containers backend) is not deployed; the command reports 'not configured' instead of dangling a dead WebSocket. Worker name `os` owns os.mino.mobi via custom_domain route (golden rule OK). CLEANUP: delete the orphan `pds-os` worker. (os/api container worker unaffected — still manual via deploy-os-api.yml.)",
   "note": "React + Vite + xterm.js terminal UI. Fully functional standalone against the user's own PDS; the optional os-api Cloudflare Containers backend (bash + claude-code over WebSocket) is enabled at build time via VITE_CONTAINER_API_URL once os-api is live. Workflow paths exclude os/api/** so frontend deploys do not fire on backend changes.",
   "paths": [
    "os/**",
    "!os/api/**"
   ],
 ...
 "unmanaged": {
  ...
  "sites": [],
  "in_progress": [
   "os/api (Cloudflare Containers worker os-mino-api; deploy-os-api.yml is READY but dispatch-only; pending: enable CF Containers, set SYNC_TOKEN+GITHUB_TOKEN secrets, create R2 bucket os-workspace, attach os-api.minomobi.com)"
  ],
  "reference_workers": [
   "workers/bsky-bot",
   "workers/cards-mint",
