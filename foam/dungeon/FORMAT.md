...
this format — step tile by tile (all six tile shapes), pass doors, light up the
...
- **Tiles sit on one global tiling** (origin `0,0`), so tiles align across
  rooms. Six shapes. Grid: centre `x=(i+0.5)·size`. Hex: pointy-top axial,
  `x=√3·R·(q+r/2)`, `z=1.5·R·r`, `R=size/√3`. The other four carry their
  polygon in `poly` (4 vertices, CCW) since no formula reproduces the tile
  from its key: **penrose** — P3 rhombs from de Bruijn's pentagrid dual
  with fixed generic offsets (one aperiodic tiling of the plane, edge
  length = `size`); **ammann** — Ammann–Beenker squares + 45° rhombs from
  the 4-grid dual (eightfold aperiodic); **seven** — sevenfold rhombs,
  three species, from the 7-grid dual; **rhombille** — the periodic
  tumbling-blocks tiling, 60°/120° rhombs paired from the triangular
  lattice. Consumers should treat `poly` as authoritative when present and
  derive adjacency by shared edges — which is how the crawl layer works
