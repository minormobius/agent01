...
this format — step tile by tile (all ten tile shapes), pass doors, light up the
...
- **Tiles sit on one global tiling** (origin `0,0`), so tiles align across
  rooms. Ten shapes. Grid: centre `x=(i+0.5)·size`. Hex: pointy-top axial,
  `x=√3·R·(q+r/2)`, `z=1.5·R·r`, `R=size/√3`. The other eight carry their
  polygon in `poly` (3–8 vertices, CCW) since no formula reproduces the
  tile from its key. Rhomb tilings (edge = `size`): **penrose** — P3
  rhombs from de Bruijn's pentagrid dual with fixed generic offsets (one
  aperiodic tiling of the plane); **ammann** — Ammann–Beenker squares +
  45° rhombs from the 4-grid dual (eightfold aperiodic); **seven** —
  sevenfold rhombs, three species, from the 7-grid dual; **rhombille** —
  the periodic tumbling-blocks tiling, 60°/120° rhombs paired from the
  triangular lattice. MULTI-SHAPE Archimedean tilings (mixed polygon
  species in one tiling, normalized so the *mean* tile area is `size²`):
  **snub** — snub square 3.3.4.3.4, ±30°-tilted squares + triangles;
  **kagome** — trihexagonal 3.6.3.6, hexagons + triangles; **rhombitri** —
  rhombitrihexagonal 3.4.6.4, triangles + squares + hexagons; **truncsq**
  — truncated square 4.8.8, octagons + 45° squares. Consumers should
  treat `poly` as authoritative when present and derive adjacency by
  shared edges — which is how the crawl layer works for every shape
  anyway.
