  ...
  return {
    ...
    tile: {
      ...
      lattice: ({
        ...
        snub: 'snub square 3.3.4.3.4, squares+triangles, mean tile area=size²; tiles carry their polygon in `poly`',
        kagome: 'trihexagonal 3.6.3.6 (kagome), hexagons+triangles, mean tile area=size²; tiles carry their polygon in `poly`',
        rhombitri: 'rhombitrihexagonal 3.4.6.4, triangles+squares+hexagons, mean tile area=size²; tiles carry their polygon in `poly`',
        truncsq: 'truncated square 4.8.8, octagons+squares, mean tile area=size²; tiles carry their polygon in `poly`',
    ...
    ...(dungeon.twin ? {
      twin: {
        ...
        seams: dungeon.twin.seams.map((s) => ({
          rooms: s.rooms.slice(), face: s.face, at: s.at.map(rnd3),
          ...(s.passable ? { passable: true } : {}),
        })),
      },
