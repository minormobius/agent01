...
export class FluoddityEngine {
  ...
  // Read back the first n particles' positions (pos.xy in clip space [-1,1]).
  // Used by the bee swarm to place a DOM link on each tracked particle.
  readEntities(n) {
    const gl = this.gl;
    const need = this.texW * this.texH * 4;
    if (!this._readBuf || this._readBuf.length < need) this._readBuf = new Float32Array(need);
    gl.bindFramebuffer(gl.FRAMEBUFFER, this.eFBO[this.ePing]);
    gl.readPixels(0, 0, this.texW, this.texH, gl.RGBA, gl.FLOAT, this._readBuf);
    const out = new Float32Array(n * 2);
    for (let i = 0; i < n; i++) { out[i*2] = this._readBuf[i*4]; out[i*2+1] = this._readBuf[i*4+1]; }
    return out;
  }
