/* idol — the seeded engine's random core. Verbatim borges pattern:

   Every girl number n yields exactly the same genome, for ever, on any machine.
   That stability is what lets /c/<n> be a permalink — she is the same person
   every time you visit, and nobody wrote her. The determinism lives here.

   mulberry32 — a tiny, fast, well-distributed 32-bit PRNG. Seeded from a string
   hash of the girl number (plus a salt, so different sub-streams of one genome —
   palette vs. hair vs. persona — don't correlate). No crypto; we only need it
   to look unrepeatable, and to be identical across reloads.
...
(function () {
  ...
  function Rand(seedStr) {
    ...
    var self = {
      ...
      // gaussian-ish in [0,1] centred on mu with spread s, clamped
      bell: function (mu, s) {
        var v = (next() + next() + next()) / 3; // approx normal 0..1 mean .5
        return Math.min(1, Math.max(0, mu + (v - 0.5) * 2 * s));
      },
      // pick one element
  ...
  I.prng = { hashStr: hashStr, mulberry32: mulberry32, Rand: Rand };
})();
