...
// ─── 2f. the analytic lenses ────────────────────────────────────────────────
{
  const preds = read('rant/crates/rant-core/src/predicates.rs');
  const css = read('rant/public/rant.css');

  // The registry is `Predicate::ALL`; the stylesheet, the docs and the API all
  // read from it. A predicate with no CSS still renders (that is the design),
  // but a *classifying* one whose tags have no colours renders monochrome and
  // silently says nothing.
  for (const t of ['pos', 'neg']) {
    check(`sentiment styles its "${t}" tag`, css.includes(`[data-t="${t}"]`));
  }
  // Not `[^;]*` — the type annotation `[&str; 10]` contains a semicolon, which
  // truncated the match to nothing and made this check pass vacuously.
  const cats = (read('rant/crates/rant-core/src/lexicon_data.rs')
    .match(/pub const NRC_CATS[^=]*=\s*\[([\s\S]*?)\]/)?.[1]
    .match(/"([a-z]+)"/g) || []).map((s) => s.replace(/"/g, ''));
  check('the NRC categories are known', cats.length === 10, cats.join(','));
  for (const c of cats.filter((c) => c !== 'positive' && c !== 'negative')) {
    check(`emotion styles "${c}"`, css.includes(`[data-t="${c}"]`));
  }

  // Colour is never the only channel — the words carry data-t in the markup and
  // are weighted, and forced-colors gets an underline.
  check('classified words are not colour-alone', /forced-colors/.test(css) && /data-t=/.test(preds));

  // Light mode gets its own steps. Reusing the dark hues puts body text at
  // ~2:1 on a light background, which the palette validator flags and which is
  // worse for text than for a chart mark.
  check('the lenses have light-mode steps of their own',
    /prefers-color-scheme: light[\s\S]*?view-emotion .w\[data-t/.test(css));

  // The lexicon tables are generated from rite's data; a stale copy is a silent
  // wrong answer rather than a build error.
  check('lexicon tables are generated, not hand-edited',
    /GENERATED by/.test(read('rant/crates/rant-core/src/lexicon_data.rs')));
  check('…and the generator is committed alongside them',
    existsSync(join(ROOT, 'rant/scripts/gen-lexicons.mjs')));
}
