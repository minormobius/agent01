...
mod route_tests {
    ...
    fn asset_paths_are_left_alone() {
        for p in ["/rant.css", "/pkg/rant_view.js", "/og/hello/card.png", "/packages/oauth-client/auth.js"] {
...
fn html(body: String, status: u16, cache: &str) -> Result<Response> {
    Ok(Response::from_html(body)?.with_status(status).with_headers(headers(&[
        ...
        ("content-security-policy", "default-src 'self'; img-src 'self' data:; style-src 'self' 'unsafe-inline'; connect-src 'self' https://auth.mino.mobi; base-uri 'none'; form-action 'self'"),
