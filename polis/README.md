...
| `hinterland.html` · `continent.js` | **The hinterlands sim — a whole continent as a closed system.** `pickContinent(world)` carves the largest connected mappa landmass; `runHinterland(world, climate, cont)` solves it over deep time on the real cell graph: climate → fertility → carrying capacity; population grows logistically and **migrates down the capacity gradient** along the graph (rivers/coasts are cheap corridors — people flow to the routes); surplus accrues as **development points**, a closed continental budget reinvested to lift the ceiling where people live. **Sea level chases the climate** — active land is `elev ≥ seaLevel(era)`, so the coast drowns in a warming and the shelf re-emerges in a glacial, and capacity + population breathe with it. **Climate shocks bite**: a volcanic winter / grand minimum is a continent-wide **harvest failure** (from `climate.events`) that crashes fertility → capacity → population, a dark age the continent slowly recovers from. **Emergent cities** (`settlements()`) sit at the density peaks — named deterministically, ranked, on the routes — linked by a **trade network** (`tradeLinks()`) heavier along coasts & rivers. Node-tested (`test/continent.selftest.mjs`, 22 checks). |
