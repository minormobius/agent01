...
- **F-06 ✅ · `b/feedgen/auth.js` is a stale fork of `packages/oauth-client/auth.js`.** It predates the scope helpers (`hasScope`/`ensureScope`), so feedgen logs in with the full `UNIFIED_SCOPE` (the long consent screen the repo is moving away from) and handles insufficient scope by full re-login. It should sync the shared lib and pass a narrow `repo:com.minomobi.feedgen.def repo:app.bsky.feed.generator` scope. (The fork exists because the ASSETS binding can't serve `../packages/` — same reason rite vendors its WASM — so "sync" means re-copy, with a header noting the provenance.) **Fixed 2026-08-01**: deleted, and feedgen now imports `/packages/oauth-client/auth.js` — the copy `deploy-b.yml` already stages into the assets dir for `/coin`, so no vendoring and no re-copy. Scope is the narrow one prescribed here. See [`ATPROTO-CONSISTENCY.md`](ATPROTO-CONSISTENCY.md) §1: the missing scope helpers were the visible half; it was also missing the sliding-session renewal and the offline grace.
