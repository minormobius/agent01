...
1. **`b/map/` — "the quarter"**: the walkable pixel-glyph catalogue (60 tools, 9 function districts, tier milestones I–IV along the road, per-tool cards with stack chips, tier pips, live-health dots and caution notes). Vanilla canvas, no deps, no build; world layout is seeded/deterministic and self-tested (all POIs reachable, no collisions).
