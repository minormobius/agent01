...
const getP = (q) => (q.kind === 'voice' ? voice[q.key] : vowelF(q.p)[q.k]);
function setP(q, v) {
...
const fmt = (q, v) => (q.kind === 'voice' ? (Math.abs(v) < 10 ? v.toFixed(3).replace(/0+$/, '').replace(/\.$/, '') : v.toFixed(0)) : v.toFixed(0));
...
function snapshot(r, change) {
  ...
  const vowels = Object.fromEntries(Object.entries(FITS.whisper).map(([p, f]) => [p, f.F.map(Math.round)]));
  const voiceNow = Object.fromEntries(GLOBAL.map((g) => [g.key, +voice[g.key].toFixed(4)]));
  progress.push({ n, minute: +minutes().toFixed(1), J: +r.J.toFixed(2), cer: +r.cer.toFixed(1), tone: +r.tone.toFixed(2), pitch: +r.pitch.toFixed(2), change, voice: voiceNow, vowels });
  ...
  writeFileSync(join(here, '..', 'lib', 'chipvoice-fit.js'), readFileSync(join(here, '..', 'lib', 'chipvoice-fit.js'), 'utf8').replace(/\nwhisper: .*,\n/, '\n').replace(/\n};\n$/, `\nwhisper: ${JSON.stringify(Object.fromEntries(Object.entries(FITS.whisper).map(([p, f]) => [p, { F: f.F.map((x) => Math.round(x)) }])))},\n};\n`));
...
if (argv.includes('--resume')) {
  ...
  snapshot(best, argv.includes('--weights') ? `phase 2: the objective reweighted (tone ×${W.tone}, pitch ×${W.pitch})${ONLY ? ', the voice controls only' : ''}` : 'resumed, with new controls');
} else {
...
const params = ONLY === 'global' ? GLOBAL : [...GLOBAL, ...VPAR];
let scale = 1;
outer: while (minutes() < MINUTES && scale > 0.12) {
  ...
  for (const q of params) {
    ...
    for (const dir of [1, -1]) {
      let v = q.kind === 'voice' ? v0 + dir * q.step * scale : v0 * (1 + dir * q.rel * scale);
      if (q.kind === 'voice') v = Math.min(q.hi, Math.max(q.lo, v));
