// main.js — Speakeasy: the page. The score is the clock, and the whole band plays it:
// the piano on pfsynth, everything else (and the city's noises) on lib/band.js.
//
//   ?t=90     a still of second 90, no audio
//   ?silent   the picture without the band

import { pianoEvents, duration, title } from './score.js';
...
const BAND = new URL('./score.js', import.meta.url).href;
const piano = new StreamPiano(pianoEvents, duration, { band: BAND });
let wallStart = null;
let finished = false;
let heldAt = null;
...
if (still !== null) {
  ...
  say('No band: the picture alone.');
} else {
  try { piano.prepare(); } catch { say('This browser will not make sound here, so the picture plays alone.'); }
  piano.onStatus = (pn) => {
    if (pn.error) { say(`The band failed to load (${pn.error.message}). Begin plays the picture alone.`); return; }
    if (pn.playing) return;
    const x = pn.speed ? `${pn.speed.toFixed(1)}× real time` : '';
    if (pn.waitingForBand) { say('Tuning the band on this device…'); return; }
    if (pn.done) say(`The band is ready: ${Math.round(pn.rendered)} s of music, made on this device.`);
...
function end() {
  ...
  say(`${title} · ${Math.round(duration)} s · a noir in cut paper, for piano, band and noisemakers`);
...
mountExtras({
  slug: 'speakeasy', title, subtitle: 'a noir in cut paper',
  events: pianoEvents, band: BAND, seconds: duration, makeRenderer, piano, ink: '#efe4cc', onInk: '#1b1816',
});
