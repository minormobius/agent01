# lab-alph — alph.minomobi.com

The first of three **lab slots**: leased tenant space for websites an agent builds
on request. Up to 100 tenants live here, each at `alph.minomobi.com/<slug>/`.
...
| Surface | `lab-alph` |
| Dir | `lab/alph/` |
| Endpoint | alph.minomobi.com |
| Type | frontend (assets-only worker, no build step) |
| Deploy | [`.github/workflows/deploy-lab-alph.yml`](../../.github/workflows/deploy-lab-alph.yml) |
...
lab/alph/
  ...
  wrangler.jsonc  worker `alph`, custom_domain alph.minomobi.com
...
A push to this surface's owning branch touching `lab/alph/**` deploys it. The
sandbox cannot reach Cloudflare — push, don't `wrangler deploy` locally. Confirm
the run logs `alph.minomobi.com (custom domain)`; green alone is not proof.
