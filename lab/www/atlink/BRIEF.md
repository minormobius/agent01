...
A single static page at `minomobi.com/atlink/` that converts between the
...
Built as a lab tenant per the task brief in the originating conversation:
...
- **Styling**: copied the dark/monospace/single-accent-colour convention from
  `lab/www/index.html` (same CSS variables, same breadcrumb pattern) rather
