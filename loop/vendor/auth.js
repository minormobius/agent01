// VENDORED COPY of packages/oauth-client/auth.js — do not edit here.
//
// The loop worker serves `assets.directory: "."`, which is `loop/` — so
// `../packages/…` is outside what it can serve and an import of it 404s at
// runtime while looking perfectly correct in the editor. Every other surface
// that uses the shared client keeps a copy for the same reason
// (hoop/vendor/auth.js, aub/web/auth.js, fluoddity/auth.js).
//
// EDIT packages/oauth-client/auth.js AND RE-COPY. A fix made here is a fix that
// exists on exactly one surface and nobody knows it.
/**
