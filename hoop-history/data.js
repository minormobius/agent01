...
const LINEAGE = [
  ...
  {v:'v099',nm:'development surface',d:'The dev bed of the first arc. Worship-oracle fixtures (divination), a Chapter One progression audit, the story spine. New work proved out here before the main site.'},
  {v:'v100',nm:'the trainer',       d:'The stable snapshot: the playable nave end-to-end, the Chapter One close fixed (Signal Chamber arrival), the roguelike combat trainer. The version the landing page links as the game.'},
  {v:'v101',nm:'the item economy',  d:'Make / mend / trade: the Smithy kernel crafts equipment from the ship’s commodities, the Lapidary reforges and tempers, a materials market + commissions open at the trade desk. Alchemy closes its loop (quaff/doff); the garden gets a real bed; the player steps onto the roamed overworld deck.'},
  {v:'v102',nm:'the living overworld',d:'The overworld gains a menace — boids bee swarms and underfoot spiders — plus the onboarding engine (little gimmes on the tier ladder). Side threads start seeking PEOPLE: waypoints track persons, not rooms.'},
  {v:'v103',nm:'NPC reform',        d:'Frozen test surface: keepers seat on their zone’s deck, Sevin relocates to the upper rind, waypoints follow the ward-unlock order and point toward the target’s deck, and authored anchor briefings give the two setter-less gates real setters.'},
  {v:'v104',nm:'THE SEVEN',         d:'planets.js becomes the unified design-language keystone: creation picks a verb, planetary alignment is GROWN from play and published as a radar; forge/brew/gems/combat rewire onto the planet language. Fungible keepers land — a gate satisfiable by ANY of several room bundles — and /plan ships as a real page.'},
  {v:'v105',nm:'seeded quest spines',d:'The current dev bed. story/weave.js casts gate keepers from the 168-room-bundle pool per world seed — every seed a different, provably progressable quest cast. All wards open after Olo’s turn-in; Factor Solen’s tier closes on a procedural murder mystery (tide/case run inside the room-bundle world); chamber errands fire every keeper’s fixture quest. /quests gains a seed roller + sweep prover.'},
];

const WINGS = [
  {role:'game · narrative time', nm:'hoop', dom:'hoop.mino.mobi', accent:'#d8a657',
   p:'The witness. A Voronoi glyph-world where every place anchors a forum thread; the deterministic ship engine (js/ship.js), HoopRoom presence, the story, the forge, the floors. The surface that visits all the others. v100 is the stable playable snapshot; v105 is the live dev bed (seeded quest spines, the murder mystery).'},
  {role:'structure · static', nm:'rind', dom:'rind.mino.mobi', accent:'#7fa8d8',
   p:'The foam space-frame shell, split out of hoop. cylinder.html sizes the shell with a Rust/WASM frame solver; foamview.html reads the layered foam + wayfinding; walk.html walks a planar cut; /brawl is combat. Since then: /ops — the OPS WEAVE, six white-collar surfaces over the eight production engines as K(6,8), with the proven one-door concourse — and /upperrind, machine halls whose flows are SOLVED (not supposed), a flux-field concourse, and the ring weave in 3D.'},
  {role:'thermodynamics · dynamical', nm:'tide', dom:'tide.mino.mobi', accent:'#6dc2b6',
   p:'The interior climate, split out of biome: the centrifugal barometer (profile), a 1-D radial atmosphere column with Mie fog optics, the rotating-frame fountain + linear sun, the ratchet topography. And now a second life as the civic wing: /goss unifies the hoop econ society into one measured web, and /case generates solvability-certified murder mysteries off it — the kernel v105 merged back into the game.'},
  {role:'ecosystem · ecological', nm:'biome', dom:'biome.mino.mobi', accent:'#8fbf7f',
   p:'The closed food-web box model (cycles/): element-exact (C·H·O·N conserve by construction), Kleiber allometry, a real-organism roster from iNaturalist/GloBI, a community-matrix stability lab with a Rust/WASM kernel. Around it: the gacha (roll a viable ecosystem), the sprite/muscle/balance labs, and /over — walk a living forest whose fauna are cast from the gacha as pixel sprites. The forge vendors the box model verbatim as life support.'},
  {role:'design study · end-on', nm:'iris', dom:'iris.mino.mobi', accent:'#a98fd8',
   p:'The end-on cross-section: looking down the axis at a small ring habitat (4 km floor inside a 5 km radiator skin). One coupled steady-state solve — lights in, radiator out — pins every temperature; switchable temperature/pressure/humidity/wind gradient views, toggleable fog + fountain jets, the inner-rim ratchet topography.'},
...
const CENSUS = [
  ...
  {p:'/docs',                   g:'core', s:'nav',    t:'The design-docs index — homepage-linked.'},
  ...
  {p:'/v099/story/spine.html',g:'story', s:'route',  t:'The flag spine: load-bearing NPC quests + deck-stacking guarantee. Now a clean /spine route on every version since.'},
  // ---- hoop: the second arc (v100–v105) ----
  {p:'/v100/', g:'story', s:'route',  t:'“The trainer” — the stable playable snapshot; the version the landing page links as the game.'},
  {p:'/v101/', g:'story', s:'route',  t:'The item economy — Smithy (make), Lapidary reforge (mend), materials market + commissions (trade); alchemy quaff/doff.'},
  {p:'/v102/', g:'story', s:'route',  t:'The living overworld — bee swarms + underfoot spiders, onboarding gimmes, person-seeking side threads.'},
  {p:'/v103/', g:'story', s:'route',  t:'NPC-reform frozen test surface — deck-aware keepers, anchor briefings, direction-aware waypoints.'},
  {p:'/v104/', g:'story', s:'route',  t:'Fungible keepers + THE SEVEN planet design language; character creation picks a verb.'},
  {p:'/v105/', g:'story', s:'route',  t:'Current dev bed — seeded quest spines, the tier-2 murder mystery, chamber errands.'},
  {p:'/v100/…/v105/ internals', g:'story', s:'buried', t:'Each second-arc version carries the full internal suite — arena, records, feed, storyboard, architecture, story spine (~36 more endpoints).'},
  {p:'/quests',      g:'story', s:'route', t:'The quest-diversity dashboard — per-gate satisfier counts, seed roller + sweep prover.'},
  {p:'/plan',        g:'story', s:'route', t:'The design-language plan (THE SEVEN) published as a real page.'},
  {p:'/cookbook',    g:'tools', s:'route', t:'The alchemist’s cookbook — the correspondence→effect grammar (humour / planet / metal / vessel). Also at /alch.'},
  {p:'/smith',       g:'tools', s:'route', t:'The Smithy testbed — craft equipment from the ship’s commodities.'},
  {p:'/garden/plot', g:'tools', s:'route', t:'The garden bed — one bed, planted in continuum, with keep-out zones.'},
  {p:'/over/demo',   g:'tools', s:'route', t:'The standing demo — a self-touring attract mode over the overworld.'},
  ...
  {p:'rind.mino.mobi/ops/',         g:'rind', s:'sib', t:'The OPS WEAVE hub — 6 white-collar surfaces × 8 production engines as K(6,8), a plain weave. 14 study pages (decks, floor, helix, nexus, office, orbit, pocket, prism, tess, tessweave…).'},
  {p:'rind.mino.mobi/ops/onedoor.html',g:'rind',s:'sib',t:'One door: two door-free concourses joined only by the 48 zero-grade K-doors — any room to any room in one door, proven by selftest.'},
  {p:'rind.mino.mobi/ops/weave.html',g:'rind',s:'sib', t:'The weave itself — the K(6,8) bipartite lattice view.'},
  {p:'rind.mino.mobi/upperrind/',   g:'rind', s:'sib', t:'The upperrind (active thread, unlinked from rind’s own landing): machine halls with a production SOLVER — flows solved, not supposed — and a flux-field concourse.'},
  {p:'rind.mino.mobi/upperrind/rings3d.html',g:'rind',s:'sib',t:'The ring weave in 3D — over/under parity, zero-grade antechambers on the crossings, a weave-tightness slider.'},
  {p:'rind.mino.mobi/upperrind/econ.html',g:'rind',s:'sib',t:'The upperrind economy view.'},
  // ---- sibling: tide (thermodynamics + the civic wing, tide.mino.mobi) ----
  {p:'tide.mino.mobi/',           g:'tide', s:'sib', t:'The THERMODYNAMICS wing landing.'},
  {p:'tide.mino.mobi/profile/',   g:'tide', s:'sib', t:'The centrifugal barometer — g(r)=ω²r + isothermal pressure profile.'},
  {p:'tide.mino.mobi/atmosphere/',g:'tide', s:'sib', t:'1-D radial column — temperature/humidity/CO₂ vs altitude, Mie fog optics.'},
  {p:'tide.mino.mobi/fountain/',  g:'tide', s:'sib', t:'Azimuthal cross-section — rotating-frame ballistic fountain + luminous-flux budget.'},
  {p:'tide.mino.mobi/ratchet/',   g:'tide', s:'sib', t:'The inner-rim ratchet topography study.'},
  {p:'tide.mino.mobi/goss/',      g:'tide', s:'sib', t:'The civic web viewer — seven webs unified into one measured society (NPCs, places, tribes, romance, the two tension axes).'},
  {p:'tide.mino.mobi/case/',      g:'tide', s:'sib', t:'Procgen murder mysteries off the civic web — solvability-certified, chain-scarred. The kernel v105 merged back into hoop.'},
  // ---- sibling: biome (ecosystem wing, biome.mino.mobi) ----
  {p:'biome.mino.mobi/',          g:'biome', s:'sib', t:'The ECOSYSTEM wing landing.'},
  {p:'biome.mino.mobi/cycles/',   g:'biome', s:'sib', t:'The closed food-web box model — element-exact, Kleiber allometry, real-organism roster (+ builder, lake, global, robustness views).'},
  {p:'biome.mino.mobi/cycles/stability.html',g:'biome',s:'sib',t:'The community-matrix stability lab (Rust/WASM kernel).'},
  {p:'biome.mino.mobi/gacha/',    g:'biome', s:'sib', t:'Roll a viable ecosystem — the pattern hoop’s econ oracle borrowed.'},
  {p:'biome.mino.mobi/sprite/',   g:'biome', s:'sib', t:'The sprite / muscle / balance labs (with /balance as its own page).'},
  {p:'biome.mino.mobi/over/',     g:'biome', s:'sib', t:'Walk a living forest — fauna cast from the gacha catalogue as pixel sprites (arthropod creeps, ambling quadrupeds).'},
  {p:'biome.mino.mobi/graph/',    g:'biome', s:'sib', t:'The food-web graph view.'},
  {p:'biome.mino.mobi/inat/',     g:'biome', s:'sib', t:'The iNaturalist/GloBI organism roster browser.'},
  // ---- sibling: iris (end-on study, iris.mino.mobi) ----
  {p:'iris.mino.mobi/',           g:'iris', s:'sib', t:'The end-on section — one coupled steady-state solve; T/P/RH/wind gradient views, fog + jets, ratchet topography.'},
...
const SECRETS = [
  {tag:'no front door', pp:'hoop/v105/worship/', h:'A working divination engine',
   p:'A full I-Ching (zhouyi + hexagrams), yarrow-stalk casting, and geomancy-in-sand — implemented as JS modules (oracle.js, oracle-cast.js, yarrow.js, scry.js, sand.js + lib/) consumed by the worship fixture. The modules have travelled version to version (v099 → v105) and there is STILL no /worship page. The oracle is real; it just has no URL.'},
  ...
  {tag:'the whole museum', pp:'/v2/ … /v105/', h:'The entire design history, served',
   p:'Twenty-one playable iterations — every fossilised decision from “the solved world” to “seeded quest spines” — sit at guessable paths. The homepage links a handful; the rest is an accidental museum nobody knows is open.'},
  {tag:'load-bearing', pp:'/v105/story/spine.html', h:'The flag spine',
   p:'A technical demo of the narrative constraint engine: NPC quests that must stay solvable, and the deck-stacking guarantee that keeps Chapter One winnable however the world re-rolls. Since v104 the proof got harder — a gate can be satisfied by any of several room bundles, and /quests sweeps seeds to prove every cast progressable.'},
  {tag:'the feedback loop', pp:'tide.mino.mobi/case/', h:'Murder mysteries, certified solvable',
   p:'Procgen whodunnits generated off the civic web — solvability-certified, chain-scarred — built as a tide study, then merged back into the game as v105’s tier-2 case with a real accusation mechanic. The first time a wing’s model returned to hoop as story content: the wings now feed BACK.'},
  {tag:'a proof, hiding', pp:'rind.mino.mobi/ops/onedoor.html', h:'Any room to any room in one door',
   p:'The ops weave re-posed as two door-free concourses joined only by the 48 zero-grade K-doors — so every room reaches every room through exactly one door, hubs included. Proven by a node selftest over the whole chamber graph. A theorem, shipped as an unlinked page.'},
];
...
const TREE = [
  ...
  {id:'items',   parent:'closed',s:'crystal',t:'make / mend / trade item economy'},
  ...
  {id:'civic',   parent:'npc',  s:'crystal', t:'goss — one unified civic web'},
  {id:'case',    parent:'civic',s:'crystal', t:'certified murder mysteries (tide/case)'},
  ...
  {id:'seven',   parent:'tabard',s:'crystal', t:'THE SEVEN — planet language'},
  {id:'verbpl',  parent:'seven', s:'tossed',  t:'creation picks verb × planet'},
  {id:'fung',    parent:'tabard',s:'crystal', t:'fungible keepers — any bundle satisfies'},
  {id:'cast',    parent:'fung',  s:'crystal', t:'seeded quest spines (weave.js)'},
  {id:'mystery', parent:'cast',  s:'dev',     t:'the murder mystery closes a tier'},
  ...
  {id:'back',   parent:'wings', s:'crystal', t:'the wings feed BACK (case → v105)'},
  {id:'shell',  parent:'oneill',s:'dev',     t:'the forge as the shell'},
  {id:'capill', parent:'shell', s:'dev',     t:'capillaries as woven surfaces'},
  {id:'onedoor',parent:'oneill',s:'crystal', t:'ops one-door concourse (proven)'},
  {id:'upper',  parent:'oneill',s:'dev',     t:'upperrind — solved machine halls'},
...
const DISCARDS = [
  ...
  {t:'creation picks verb × planet', k:'the verb pick alone. A player choosing their planet up front made the alignment a costume; now planetary alignment is GROWN from what you actually do and published as a radar.'},
];
