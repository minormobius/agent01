...
const ALLOWED_ORIGINS = [
  ...
  'https://parts.mino.mobi',       // communities of CAD parts — mounted at cad.mino.mobi/parts/ today (zone domain ceiling); kept for the day it gets its own host
];
...
async function handleClientMetadata(env: Env): Promise<Response> {
  ...
  const metadata = {
    ...
    // declared here, so this must remain a superset. (io.ticket is included
    // via WRITE_COLLECTIONS in scope.ts for the atproto tracker portal.)
