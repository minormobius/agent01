...
(function () {
  ...
  function renderTale() {
    ...
    t.passages.forEach((pass, pi) => {
      ...
      if (C.themes) { const th = C.themes.filter((x) => x.passage === pi + 1); if (th.length) body.appendChild(el("div", "tale-themes", th.map((x) => "\u27dc <strong>type-scene:</strong> " + escapeHtml(x.label)).join(" &nbsp;\u00b7&nbsp; "))); }
  ...
  function renderPropp() {
    ...
    if (C.themes && C.themes.length) { const td = el("div", "propp-themes"); td.innerHTML = '<div class="pt-head">Oral type-scenes <span>\u00b7 Parry\u2013Lord</span></div>' + C.themes.map((x) => `<a class="pt-row" data-passage="${x.passage}"><span class="pt-id">\u27dc ${escapeHtml(x.label)}</span><span class="pt-note">${x.note}</span></a>`).join(""); $("#propp-intro").after(td); }
  ...
  function buildMythograph() {
    ...
    (C.themes || []).forEach((th, i) => add("th-" + i, "Type-scene \u00b7 " + th.label, "\u27dc", "theme", { passage: th.passage }, th.note || ""));
    if (C.desire) add("de-obj", "The Object of desire \u2014 " + C.desire.object, "\u2726", "desire", { tab: "desire" }, (C.desire.value ? C.desire.value + ". " : "") + (C.desire.note || ""));
    ...
    (C.themes || []).forEach((th, i) => edge("th-" + i, "mv-" + th.passage, "stages"));
    if (C.desire) { const A = C.desire, used = {}; const link = (ref, type, dashed) => { if (ref && !used[ref]) { used[ref] = 1; edge("ch-" + ref, "de-obj", type, dashed); } }; link(A.subjectRef, "desires", A.unreachable); link(A.opponentRef, "actant"); link(A.senderRef, "actant"); (A.helpers || []).forEach((h) => link(h.ref, "actant")); }
  ...
  function renderMythograph() {
    ...
    [["spine", "narrative spine (I → XII)"], ["appears", "character → movement"], ["relates", "character ↔ character"], ["exhibits", "motif → movement"], ["realizes", "function → movement"], ["stages", "type-scene → movement"], ["desires", "Subject → Object (desire)"], ["actant", "actant → Object"]]
  ...
  let proppDrawn = false, webDrawn = false, mythDrawn = false, desireDrawn = false, current = "read";
