/* Sir Gawain and the Green Knight — standalone site reader. */
(function () {
  "use strict";
  ...
  function renderCharacters() {
    const ch = G.characters; if (!ch) return;
      ...
      members.forEach((c) => {
        ...
        if (c.appears && c.appears.length) {
          ...
          c.appears.forEach((n, i) => {
            const a = el("a", null, "Fitt " + toRoman(n)); a.setAttribute("data-passage", n); a.title = (G.tale.passages[n - 1] || {}).title || "";
  ...
  function renderWeb() {
    ...
    const nodeEls = [];
    nodes.forEach((n, i) => {
      const r = (n.role === "principal") ? 13 : (n.role === "camelot" ? 10 : 9);
  ...
  function renderPropp() {
    ...
    moves.forEach((m, i) => {
      ...
      const pass = G.tale && G.tale.passages[m.passage - 1];
    ...
    ab.appendChild(el("h3", null, "What the poem bends, skips, or fuses"));
  ...
  /* ====================== THE MYTHOGRAPH (synergistic graph) ======================
     One typed multigraph over the whole annotation layer. Nodes: fitt,
     character, motif, propp-function. Edges: APPEARS_IN (char->fitt),
     RELATES_TO (char<->char), EXHIBITS (motif->fitt), REALIZES (fn->fitt).
     The four Fitts are pinned along a horizontal spine; everything else is
  ...
  function buildMythograph() {
    ...
    G.tale.passages.forEach((p, i) => { const n = i + 1; add("mv-" + n, p.title, toRoman(n), "movement", { passage: n }, (p.segments[0] || {}).e || ""); });
  ...
  const MYTH_TYPE = {
    movement: { color: "#c9a24a", label: "Fitts", r: 17 },
  ...
  function renderMythograph() {
    ...
    function fillDetail(i) {
      ...
      if (n.link.passage) { const a = el("a", null, "→ Read this Fitt"); a.setAttribute("data-passage", n.link.passage); open.appendChild(a); }
