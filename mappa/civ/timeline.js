...
const WEIGHT = {
  ...
  climate: 85, polityRise: 80, polityFall: 78, collapse: 74, belief: 70, eminence: 66, war: 62,
  demography: 60, schism: 58, stateFormation: 50, crisis: 48, institution: 45,
  conversion: 44, boom: 40, resource: 38, split: 36, extinction: 34,
...
export function buildTimeline(ch, mode) {
  ...
  const entries = [];
  const add = (t, kind, title, body, refs) => entries.push({ t, year: yr(t), kind, title, body, refs: refs || {} });
    ...
    switch (e.type) {
      case 'founding':
        add(e.t, 'founding',
          ...
          great ? `${e.pop} souls of the ${cn} settle their homeland on landmass ${e.landmass}.`
                : `${e.pop} foragers, one habitable nucleus, landmass ${e.landmass}: initial conditions, not intentions, set what follows.`,
      ...
      case 'agriculture':
        add(e.t, 'agriculture',
          ...
          great ? `the ${cn} bind themselves to the soil — the ${e.package} package — and their children multiply.`
                : `on landmass ${e.landmass}, subsistence crosses the ${e.package} threshold: surplus, storage, sedentism — a phase transition no one chose.`,
      ...
      case 'industry':
        add(e.t, 'industry',
          ...
          great ? `the ${cn} harness machine and coal; their reach outruns every rival's.`
                : `on landmass ${e.landmass}, capital begins compounding: energy per head decouples from land per head.`,
      ...
      case 'beliefFounded': {
        ...
        add(e.t, 'belief',
          ...
          { belief: e.belief, beliefName: e.name, culture: e.culture, cultureName: cn, register: e.register, doctrine: b ? b.doctrine : undefined });
        break;
      }
      case 'schism':
        add(e.t, 'schism',
      ...
      case 'polityRise': {
        ...
        add(e.t, 'polityRise',
          ...
          { culture: e.culture, cultureName: cn, inst: st ? st.id : undefined, seat: e.seat });
        break;
      }
      case 'polityFall':
        add(e.t, 'polityFall',
      ...
      case 'admixtureSpike':
        ...
        break;
      case 'techUnlock':
        if (great) add(e.t, 'tech', `the ${cn} master ${e.cap}`, `the ${e.how === 'diffusion' ? `craft of ${e.cap} reaches the ${cn} along the roads` : `${cn} work out ${e.cap} for themselves`} (${e.tier}).`, { culture: e.culture, cultureName: cn, cap: e.cap, tier: e.tier, how: e.how });
        else add(e.t, 'tech', `the ${e.cap} frontier advances`, `${e.cap} (${e.tier}) ${e.how === 'diffusion' ? 'diffuses along contact networks' : 'is independently invented'} — ideas move faster than peoples.`, { cap: e.cap, tier: e.tier, how: e.how });
        break;
      case 'institutionFounded': {
        if (great) { const o = instById.get(e.inst); add(e.t, 'institution', `the founding of ${e.name}`, `${o && o.leader ? o.leader + ' charters ' : ''}${e.name} among the ${cn}.`, { inst: e.inst, instName: e.name, kind: e.kind, culture: e.culture, cultureName: cn }); }
        break;
      }
  ...
  // ---- budget: trim least-significant, keep chronological -----------------------
  // same-tick tiebreak is ASCENDING weight so the heavyweight summary entries
  // (rulesets, then closing) end the timeline rather than preceding lighter news
