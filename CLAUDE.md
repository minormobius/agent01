...
| [`packages/figure/`](packages/figure/) | a character rig in head units: proportions from a few numbers, poses written as intent, a walk that cannot slide, an anime cel-and-ink WebGL2 renderer, and node checks (slip, reach, interpenetration, balance). To draw a figure as an agent: [`packages/figure/SKILL.md`](packages/figure/SKILL.md). `figure.selftest.mjs` + `browser.selftest.mjs` gate it |
