...
| `brut/stair.js` | **the stair** — pure solver and typology. `solveFlight` (equal risers, Blondel, pitch), `stairFootprint` (how big a shaft it needs — takes a LIST of storey heights and returns the envelope), `layout` (flights, landings, every tread), `stairParts`, `stairPlan` (the plan symbol), `check`, `chooseStair`. Ten types across three ways of spending the horizontal length: run, fold, turn. |
...
| `brut/arch.selftest.mjs` | **run this before touching the kernel**: `node tjs/brut/arch.selftest.mjs` (266 checks, ~6 s). It is also a gate in `deploy-tjs.yml`. |
...
17. **Stairs reach the ground by construction.** One stair per shaft per level,
