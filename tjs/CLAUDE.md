...
| File | Role |
|---|---|
| `manifold/shell.js` | **the geometry** — `deriveParams` / `resolveParams` / `paramsToQuery` (the permalink codec), `generate()` (legs → ring → mouths), `memberParts()`, `surfaceGeometry()`, `profile()`. Pure, DOM-free, no three.js. |
...
| `manifold/shell.selftest.mjs` | **run this before touching either kernel**: `node tjs/manifold/shell.selftest.mjs` (83 checks, ~14 s). It is also a gate in `deploy-tjs.yml`. |
...
| `brut/arch.selftest.mjs` | **run this before touching the kernel**: `node tjs/brut/arch.selftest.mjs` (62 checks, ~4 s). It is also a gate in `deploy-tjs.yml`. |
