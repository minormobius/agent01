...
**This bench is someone else's idea, used with permission and credited in five
places. Do not remove any of them.** The concept and the photogrammetry-fitted
part dimensions are **Codetaur**'s
([@vibe-coded.com](https://bsky.app/profile/vibe-coded.com) ·
[github.com/ngwnos/marbles](https://github.com/ngwnos/marbles), MIT © 2026
ngwnos · [the original](https://marbles.vibe-coded.com/)). The credit appears in
the page crumb, in a panel block, in the `marbleworks.js` header, in the
selftest header, and in full in [`marbles/CREDITS.md`](marbles/CREDITS.md),
which carries the MIT text verbatim. A change that drops one of those is a
regression, not a tidy-up.

The brief that produced this bench was **mobile support** for that concept.

### Files

| | |
|---|---|
| `marbles/marbleworks.js` | the whole model — spec, port table, catalogue, snap, solver, presets, permalink codec. Pure: no three.js, no DOM |
| `marbles/index.html` | renderer + touch UI. Imports the module; never redefines a dimension |
| `marbles/marbles.selftest.mjs` | 213 known-answer checks. **Gates the deploy** |
| `marbles/CREDITS.md` | the accreditation and the MIT notice |

### The grid, which is the ported part

Every mating elevation is a multiple of **rise = 47 mm**; `insertion = 15 mm` is
how far a male tip enters the socket above, and the *shoulder*, not the tip, is
the stacking datum. A connector column at piece-local bottom `b` exposes a
**female at `b`** and a **male at `b + 47`**; two pieces connect when one's male
sits in the other's female, which puts the female-side piece exactly one rise
higher. That single rule is the entire assembly model.

A trough enters at `b + 30` (shoulder − 17 mm channel depth) and leaves at
`b + 17`, having fallen the fitted `runDrop = 13 mm`. The piece below is one
rise down, so its inlet cup is 34 mm under the outlet: the marble free-falls
that 34 mm through the 21 mm bore, which the 15.9 mm marble clears. Plan
geometry is one chord — `PORT_SPAN = hypot(138, 24) = 140.0714 mm` — for the
two-column pieces *and* for the three-column ones, whose ports sit on a radius
of 80.87 mm at 120°.

### The solver, which is ours

A marble is a solid sphere rolling without slip on the trough's flat floor
(`MARBLE.radius 7.95 > TRACK.fillet 5.5`, so it never wedges into the corner
arcs), giving `a = (5/7)·g·sinθ` minus rolling resistance and a small quadratic
drag. One arclength coordinate per marble, fixed 240 Hz step, exit speed
de-quantised at each segment end so the integrator actually converges. That is
the trade against the original's Box3D rigid-body solve, and it is why this runs
at 60 fps on a phone with no wasm. **It cannot model marble-on-marble contact or
a marble leaving its trough** — say so rather than papering over it.

### Mobile is the point, so these are load-bearing

- `renderer.setSize(w, h)` — **not** `setSize(w, h, false)`. With `updateStyle`
  off the canvas keeps its device-pixel attribute size as its CSS size and
  overflows the stage on every DPR > 1 device.
- The stage is flexed (`body` is a column flex, `.stage-row` is `flex:1`), never
  `calc(100% − 39px)`: the credit line wraps to a second row on a phone and any
  hard-coded chrome height pushes the canvas off-screen.
- `frameBoard()` fits the **narrower** of the two half-angles — a portrait phone
  is limited horizontally — and moves the fog with the camera, or the whole
  model fogs into the background at phone framing distances.
- Picking is **screen-space**, not a raycast: a 27 mm post at full-board zoom is
  a couple of device pixels across, and no ray precision makes that tappable.
- Portrait gets a bottom sheet; landscape (`max-height:560px`) switches to a
  side panel, because a sheet on a landscape phone leaves ~130 px of run.

### Adding a part

Add its columns to `PART_PORTS` (plan x/z and local bottom, `post:false` for a
terminal), a `CENTRELINES` generator, and a `P(...)` row in `PARTS`. The
selftest then asserts, for free, that its runs start at `bottom + 30` over the
inlet column, end at `bottom + 17` over the outlet column, descend
monotonically, and carry no degenerate samples. If it fails those, the part is
wrong, not the test.
