...
// Pick a DISTINCT interior anchor node for one agent at one stop room (v091): instead of every
// commuter targeting the shared DOOR node — where they all stacked — each agent hashes to one of the
...
// per-agent interior anchors [home, work, third]; their sprite is roled to their JOB.
export function buildResidents(world, walk, society, opts = {}) {
  ...
  for (const { home, work, third } of byHome.values()) {
    ...
    if (stops.length < 2) continue;                          // nowhere to commute
    ...
    people.forEach((name, i) => {
      ...
      const route = stops.map((s) => anchorNode(id, s, walk));
      ...
      agents.push({ id, name, role, glyph: (work || home).glyph, featureKey: home.gid + '#' + i, homeKey: home, workKey: work, thirdKey: third,
        ...
        rng, genome: buildGenome('hoop-resident:' + id, { role, size }), x, y, bx: x, by: y, sepx: 0, sepy: 0, dir: 'S' });
    });
  }
  return agents;
}

// Advance residents by `dtMs` in two phases (v091). PHASE 1 glides each in-radius agent's CLEAN base
// position (bx,by) along its leg, then dwells, then plans the next leg. PHASE 2 applies boids-style
// SEPARATION over the in-radius set — a clamped push apart so residents never trample or stack, on the
// concourse or in a shared room — writing the rendered (x,y) = (bx,by)+sep. Keeping the base separate
// from the rendered point means the nudge can't accumulate or drift the commute off the walk graph.
// `sep` is the personal-space radius and `sepMax` the max displacement (both world units; the page
// scales them to playerW). Far agents stay frozen, untouched, exactly as before.
export function stepResidents(agents, walk, dtMs, { cx = 0, cy = 0, radius = Infinity, separate = true, sep = 5, sepMax = 3 } = {}) {
  ...
  for (const a of agents) {
    ...
    if (i >= a.pts.length - 1) {                             // arrived → dwell, then next leg
      ...
      a.bx = A[0] + (B[0] - A[0]) * ft; a.by = A[1] + (B[1] - A[1]) * ft;
    }
  }
  if (separate && active.length > 1) applySeparation(active, sep, sepMax);
  else for (const a of active) { a.sepx = 0; a.sepy = 0; a.x = a.bx; a.y = a.by; }
  return agents;
}

// Boids "separation" over the active set: a bucket grid (cell = sep) then, per agent, sum the inverse
// pushes from neighbours within `sep`, clamp the displacement to `sepMax`, and offset the render point.
// Coincident agents get a deterministic tie-break so two stacked on one cell still part.
function applySeparation(active, sep, sepMax) {
  const sep2 = sep * sep, grid = new Map(), key = (gx, gy) => gx + ',' + gy;
  for (const a of active) { const k = key(Math.floor(a.bx / sep), Math.floor(a.by / sep)); let b = grid.get(k); if (!b) grid.set(k, b = []); b.push(a); }
  for (const a of active) {
    let dx = 0, dy = 0; const gx = Math.floor(a.bx / sep), gy = Math.floor(a.by / sep);
    for (let oy = -1; oy <= 1; oy++) for (let ox = -1; ox <= 1; ox++) {
      const b = grid.get(key(gx + ox, gy + oy)); if (!b) continue;
      for (const o of b) {
        if (o === a) continue;
        const ex = a.bx - o.bx, ey = a.by - o.by, d2 = ex * ex + ey * ey;
        if (d2 === 0) { dx += (a.id < o.id ? 0.5 : -0.5); continue; }      // perfectly coincident → split
        if (d2 >= sep2) continue;
        const d = Math.sqrt(d2), w = (sep - d) / sep; dx += (ex / d) * w; dy += (ey / d) * w;
      }
    }
    const m = Math.hypot(dx, dy);
    if (m > 1e-6) { const s = Math.min(sepMax, m * sep * 0.5) / m; a.sepx = dx * s; a.sepy = dy * s; } else { a.sepx = 0; a.sepy = 0; }
