// hoop/over/eden.js — the biosphere generator for level zero. Pure, deterministic, node-testable.
//
...
export function makeEden(seed = 7, opts = {}) {
  ...
  function tileTrees(az, ax) {
    ...
    for (const [px, py] of cands) {
      ...
      const rho = wetness(px, py), rad = lerp(46, 17, rho), cx = Math.floor(px / CELL), cy = Math.floor(py / CELL); let ok = true;
