...
export function bakeFog(rec, isSeen, isInView) {
  ...
  const path = (p) => { ctx.beginPath(); ctx.moveTo(p[0][0] - ox, p[0][1] - oy); for (let i = 1; i < p.length; i++) ctx.lineTo(p[i][0] - ox, p[i][1] - oy); ctx.closePath(); };
  let drew = false;
  for (let i = 0; i < rec.cells.length; i++) { const c = rec.cells[i]; if (c.poly.length < 3) continue; const s = isSeen(i), v = isInView(i), a = !s ? 0.93 : v ? 0 : 0.5; if (a <= 0) continue; drew = true; ctx.fillStyle = `rgba(6,8,11,${a})`; path(c.poly); ctx.fill(); }
  // PERF: a fully-revealed, fully-in-view chunk bakes an all-transparent tile — flag it so the caller
  // can skip both the blur below and the per-frame full-tile blit (pure fill-rate, painting nothing).
  if (!drew) return { canvas: null, empty: true, ox, oy, w: W, h: H };
