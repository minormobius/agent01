...
function genFix(st, nav, npcId, threadKey, salt) {
  ...
  return {
    ...
    steps: [
      { key: lensA, label: `cross-check the ${st.geo.warps[+lensA.slice(1)].id} lens` },
      { key: lensB, label: `…against the ${st.geo.warps[+lensB.slice(1)].id} lens` },
      { key: site, label: `reach the fault in ${weaveLabel(st, site)}` },
    ],
    ordered: true,
