...
# The PTY spawns `bash --login` (server.js), and a LOGIN shell reads
# /etc/profile + ~/.bash_profile — NOT ~/.bashrc, which is where everything
# above lives. Without this bridge a raw `container` session lands on a bare
# `bash-5.x$` with no os.mino prompt, no banner, no `work`, no `k` — the shell
# looks broken because none of its advertised furniture is there. `kimi` hides
# the same bug by booting straight into the agent, so it only shows up on the
# raw path (and on the bash you fall back to when an agent exits).
#
# Written every boot rather than guarded: .bash_profile is deliberately NOT in
# server.js's sync tar, so it does not come back with the workspace.
cat > "$HOME/.bash_profile" << 'PROFILE'
# os.mino — generated by startup.sh every boot; edit ~/.bashrc instead.
# A login shell does not read ~/.bashrc on its own. Bridge it.
