// packages/dataviz — stats.js: a dependency-free statistics core.
//
// CANONICAL SOURCE. Consumers that serve this file as a static asset keep a
// synced copy (e.g. wormhole/stats.js); `node scripts/sync-dataviz.mjs --check`
// enforces that the copies are byte-identical, and `--write` refreshes them.
// Edit here, never in a copy.
//
// Pure functions, no state, no build step. Attaches to globalThis as
// WORMHOLE_STATS so the same file works via <script src>, via `import` for
// side effects in a Worker, and in node. Module consumers can instead use the
// facade: `import { stats } from '../../packages/dataviz/index.mjs'`.
//
// Descriptive: sum, mean, variance, sd, min, max, quantile, median, histogram,
//   ecdf, kde (Gaussian, Silverman bandwidth), rank.
// Inference:   correlation, spearman, corrP, normalCdf, normalQuantile, anova,
//   chiSquare, logRank.
// Models:      ols (k predictors, normal equations + Gaussian elimination, with
//   standard errors, R² and AIC), logistic, poisson, lda, kaplanMeier, roc.
// Structure:   jacobiEig, pca, cmdscale (classical MDS + stress), kmeans
//   (k-means++), hclust (average linkage), communities (label propagation +
//   modularity), mahalanobis, euclid.
// Signal:      detrend, periodogram (DFT), acf, changepoints (binary segmentation).
//
// Every estimator here is checked against a known-answer case in
// packages/dataviz/dataviz.selftest.mjs — run it before changing any of them.
