// packages/dataviz — charts.js: a publication-quality SVG chart library.
//
// CANONICAL SOURCE. Consumers that serve this file as a static asset keep a
// synced copy (e.g. wormhole/charts.js); `node scripts/sync-dataviz.mjs --check`
// enforces that the copies are byte-identical, and `--write` refreshes them.
// Edit here, never in a copy.
//
// Dependency-free, deterministic, renders to SVG strings — so it works in a
// Worker, in the browser, and in node (server-side figures, snapshot tests).
// No build step. Requires stats.js (WORMHOLE_STATS) to be loaded first;
// attaches to globalThis as WORMHOLE_CHARTS. Module consumers can use the
// facade: `import { charts } from '../../packages/dataviz/index.mjs'`.
//
// Design follows the dataviz method: colour assigned by job, not cycled;
// Okabe–Ito colourblind-safe categorical palette (validated with the dataviz
// skill's checker), viridis sequential, blue–gray–red diverging with a neutral
// midpoint; thin marks, recessive axes, legends placed in the emptiest quadrant,
// direct value labels only where they help.
//
// Charts (24): scatterFit · violin · box · ridgeline · histogram · groupedBar ·
// heatmap · waterfall · forest · qq · line · spectrum · scree · biplot ·
// clusterScatter · dendrogram · roc · kaplanMeier · lollipop · logisticCurve ·
// stem · stackedBar · network · hexbin.
// Each takes {…, width, height} and returns an <svg> string.
