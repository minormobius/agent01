...
| `poly.js` | `Poly` — a polycrystal: several tilings, each a grain turned by a whole-degree angle and set down at its own place, sharing one stack of layers; a Prism over their composite, with the overlap relation between grains as exclusion, foreign walls in the terrace rule, rays per grain, and arrival from one melt; `grainsFor(seed, spec)` draws the grains |
