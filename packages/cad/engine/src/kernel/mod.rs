...
impl Geom {
    ...
    /// the index is wrong.
    pub fn fits(&self, n: [f64; 3], _c: [f64; 3], pts: &[[f64; 3]]) -> Option<f64> {
        let dot = |a: [f64; 3], b: [f64; 3]| a[0] * b[0] + a[1] * b[1] + a[2] * b[2];
        match self {
            Geom::Plane { normal, point } => {
                if dot(n, *normal).abs() <= 0.99 { return None; }
                // every point of the face must lie in the plane
                let scale = pts.iter().map(|p| (p[0] - point[0]).abs() + (p[1] - point[1]).abs() + (p[2] - point[2]).abs()).fold(1e-6, f64::max);
                let worst = pts.iter().map(|p| dot([p[0] - point[0], p[1] - point[1], p[2] - point[2]], *normal).abs()).fold(0.0, f64::max);
                if worst > 1e-3 * scale { return None; }
                Some(dot(n, *normal).abs())
            }
            Geom::Cylinder { axis, center, radius } => {
                if *radius <= 1e-9 || dot(n, *axis).abs() > 0.2 || pts.is_empty() { return None; }
                // every point of the face must lie on the cylinder. The
                // centroid alone cannot do this: a wedge's centroid sits
                // inside its own radius, so two nearby radii both "fit" and
                // the closer wrong one wins (measured on a flanged nut's
                // 8.4 bore and 10 neck, 2026-09-13).
                ...
                for p in pts {
                    ...
                    worst = worst.max((len - radius).abs());
                }
                if worst > 0.02 * radius { return None; }
                Some(1.0 - worst / radius)
            }
        }
    }
}
...
pub fn geom_for(geoms: &[Option<Geom>], i: usize, normal: [f64; 3], centroid: [f64; 3], pts: &[[f64; 3]]) -> Option<Geom> {
    if let Some(Some(g)) = geoms.get(i) {
        if g.fits(normal, centroid, pts).is_some() { return Some(g.clone()); }
    }
    let mut best: Option<(f64, &Geom)> = None;
    for g in geoms.iter().flatten() {
        if let Some(score) = g.fits(normal, centroid, pts) {
