...
// expected touches: listed, never failed on. Reference components are left out.
...
const SYMBOL = { collision: '✗', expected: '~', close: '!', clear: '✓' };
const verdict = (p) => SYMBOL[verdictOf(p, expected, clearance)];
const row = (p) => `${verdict(p)} ${p.a} × ${p.b}  ${p.penetration > 0 ? `${p.contained ? p.contained : 'crossing'}, ${f4(p.penetration)} mm deep` : p.touching ? 'touching' : `${f4(p.distance)} mm`}${p.t !== undefined ? ` at t = ${p.t.toFixed(3)} s` : ''}${verdict(p) === '~' ? '  (expected touch)' : ''}`;
if (!sweep) {
...
process.exit(out.pairs.some((p) => verdict(p) === '✗' || verdict(p) === '!') ? 1 : 0);
