// Manifold: mesh booleans. Consumes the engine's sampled polylines (it has no
// exact curves), extrudes and revolves CrossSections, and unions/cuts. No
// faces, no STEP.
...
function frameMat(f) {
  // column-major 4x4 from frame axes; local (x,y,z) → world
  return [f.u[0], f.u[1], f.u[2], 0, f.v[0], f.v[1], f.v[2], 0, f.n[0], f.n[1], f.n[2], 0, f.o[0], f.o[1], f.o[2], 1];
}
...
export function make() {
  ...
  return {
    id: 'manifold',
    exact: false,
    bytes: fs.statSync(path.join(here, '..', 'node_modules', 'manifold-3d', 'manifold.wasm')).size,
    async init() {
      const t0 = performance.now();
      wasm = await Module(); wasm.setup(); ({ Manifold, CrossSection } = wasm);
      ...
      try {
        for (const op of resolved.resolved.ops) {
          ...
          if (op.op === 'extrude') {
            ...
            if (Math.abs(op.angle_deg - 360) > 1e-9) return { ok: false, error: { op: op.id, msg: 'partial revolve not wired for manifold', unsupported: true }, ms: performance.now() - t0 };
            const d = op.axis_d, l = Math.hypot(d[0], d[1]); const ad = [d[0] / l, d[1] / l];
            let perp = [-ad[1], ad[0]];
            // side of the axis: from the point farthest off it
            ...
            // profile in (r, h) coordinates; Manifold revolves x=r about Y then maps Y→Z
            ...
            const m = new CrossSection(prof, 'EvenOdd').revolve(256);
        ...
        const np = mesh.numProp; const pos = [];
        for (let i = 0; i < mesh.vertProperties.length; i += np) pos.push([mesh.vertProperties[i], mesh.vertProperties[i + 1], mesh.vertProperties[i + 2]]);
        const tris = []; for (let i = 0; i < mesh.triVerts.length; i += 3) tris.push([mesh.triVerts[i], mesh.triVerts[i + 1], mesh.triVerts[i + 2]]);
        const ms = performance.now() - t0;
        return { ok: true, ms, mesh: { pos, tris }, faces: [], step: null, kernelVolume: body.volume(), genus: body.genus() };
      } catch (e) {
