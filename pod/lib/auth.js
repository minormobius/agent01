// VENDORED COPY of packages/oauth-client/auth.js — pod is a no-build assets
// worker that can only serve files under pod/, so the shared client lib is
// colocated here. The confidential OAuth client is still the shared worker at
// auth.mino.mobi; this is only the browser-side lib. Keep in sync with source.
