...
export async function getRecord(did, rkey) {
...
export async function links(target, path, limit = 100, cursor = null) {
...
// Enumerate every organism via the gallery anchor, hydrating records from each
// author's PDS. onProgress(loaded, total) is called as records arrive.
export async function enumerateAll(onProgress) {
  const all = [];
  let cursor = null, total = 0;
  do {
    const res = await links(ANCHOR, '.gallery', 100, cursor);
    ...
    await Promise.all(Array.from({ length: CONC }, async () => {
      while (list.length) {
        const lr = list.shift();
        try { all.push(await getRecord(lr.did, lr.rkey)); } catch { /* skip unreadable */ }
