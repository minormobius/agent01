...
// obsidian: where water meets lava. Only a diamond pick gets it out.
def('obsidian',       { hard: 50, tool: 4, color: '#2b1d3d', top: '#3a2852' });
...
export const RECIPES = {
  ...
  diamond_sword:   { n: 1, need: { diamond: 2, stick: 1 }, at: 'crafting_table' },
  iron_armor:      { n: 1, need: { iron_ingot: 8 }, at: 'crafting_table' },
...
// v3: diamonds near bedrock, and lava pockets deep in the rock of every world
// kind (a pool with a pocket of air over it, sealed in stone). Their own rng,
// so everything v2 placed is where it was.
function deepPass({ seed, tiling, blocks, height }) {
  const cols = tiling.cols, N = cols.length, rng = mulberry(hash32(seed, 0xD1A));
  const at = (c, y) => blocks[c * H + y];
  for (let v = 0; v < Math.round(N / 180); v++) {
    let c = Math.floor(rng() * N), y = 1 + Math.floor(rng() * 5);
    for (let k = 0, len = 1 + Math.floor(rng() * 3); k < len; k++) {
      if (at(c, y) === B.stone) blocks[c * H + y] = B.diamond_ore;
      if (cols[c].adj.length) c = cols[c].adj[Math.floor(rng() * cols[c].adj.length)];
    }
  }
  for (let v = 0; v < Math.round(N / 450); v++) {
    const c0 = Math.floor(rng() * N);
    if (height[c0] < 12) continue;
    const pool = [c0, ...cols[c0].adj];
