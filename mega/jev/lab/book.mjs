...
export function newBook({ seed = 1, costs = {} } = {}) {
  const c = { ...DEFAULT_COSTS, ...costs };
  const mk = () => ({ pos: 0, equity: 1, turnover: 0, fills: 0, costPaid: 0 });
  return {
    costs: c,
    rnd: mulberry(seed),
    lastPx: null,
 ...
 */
export function step(book, { px, spreadBps = 0, action = null, t = Date.now(), meta = {} }) {
  ...
  const wantJev = decided ? targetPosition(action, book.jev.pos) : book.jev.pos;
  // The control trades exactly as often as Jev does, so any difference is
  // the decisions and not the trading frequency.
  const wantRand = decided
    ? targetPosition(ACTIONS[Math.floor(book.rnd() * ACTIONS.length)], book.rand.pos)
    : book.rand.pos;
  // Buy-and-hold is long from the first tick and never moves again.
  const wantHold = 1;
...
export function summary(book) {
  ...
  return {
    ...
    verdict: diffs.length < 30 ? 'too few decisions to say anything'
