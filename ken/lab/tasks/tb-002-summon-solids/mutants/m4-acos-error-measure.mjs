/* SEEDED DEFECT: acos instead of atan2 of the cross product. Ill-conditioned near zero, so exact geometry reports about 1e-6 degrees of pure noise. */
...
export function verify(con, { tolDeg = 1e-6, tolSpread = 1e-9 } = {}) {
  ...
  for (let i = 0; i < want.length; i++) {
    ...
    maxErr = Math.max(maxErr, Math.acos(Math.min(1, Math.abs(dot3(a, b)))) * 180 / Math.PI);
  }
