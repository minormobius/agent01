/* SEEDED DEFECT: extent measured under the metric instead of in space. clearanceNeeded() then
   overstates the room a summon needs, so legal placements are refused. Neither check looks at
   extent or clearance, which is why this one should survive: a coverage hole, reported. */
...
export function constellation(solid, { centre = [0, 0, 0], r = 1.6, aniso = 2.2, rotate = 0 } = {}) {
  ...
  for (const n of neighbours) {
    ...
    extent = Math.max(extent, Math.hypot(d[0], d[1] * aniso, d[2]));
  }
