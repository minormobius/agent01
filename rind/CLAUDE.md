...
node rind/upperrind/ringpocket.selftest.mjs        # ★ the RING TOPOLOGY WALKED (pocketweave `ringMode`, kernel ops/ringpocket.js): the schematic brought into the pocket dimension — RA/RR are annular ring pockets that CLOSE INTO A LOOP (the last arc's seam stitches back to the first), each touching all 12 threads; the fulfillment nexus (NX) bonded to RA; threads grow inner-RA + outer-RR doors. Default off ⇒ ops/pocket unchanged. The upperrind map (app.js) walks it.
