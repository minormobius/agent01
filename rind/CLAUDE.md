...
node rind/upperrind/ringpocket.selftest.mjs        # ★ the RING TOPOLOGY WALKED (pocketweave `ringMode`, kernel ops/ringpocket.js): the schematic brought into the pocket dimension — RA/RR are annular ring pockets that CLOSE INTO A LOOP (the last arc's seam stitches back to the first), each crossing all 12 threads THROUGH A ZERO-GRADE ANTECHAMBER ('ZA:'/'ZR:' — the NO-LADDER rule, same as the weave's X interfaces; never a direct floor-to-floor door); the fulfillment nexus (NX) bonded to RA. Default off ⇒ ops/pocket unchanged. The upperrind map (app.js) walks it.
