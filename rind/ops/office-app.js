...
function buildOffice(t) {
  ...
  for (const [gi, d] of t.doorAt) {
    ...
    for (let h = 0; h < q.length; h++) { const dep = seen.get(q[h]); if (dep >= 1) continue; for (const nb of stepNbrs(q[h], N)) if (!seen.has(nb)) { seen.set(nb, dep + 1); q.push(nb); } }   // the FIRST chamber only
...
// ── camera: fit the current thread + its visible first-chamber peeks ──
function fitThread() {
  const t = curThread(); let x0 = 1e9, y0 = 1e9, x1 = -1e9, y1 = -1e9;
  const add = (gi) => { const c = cells[gi]; x0 = Math.min(x0, c.x); y0 = Math.min(y0, c.y); x1 = Math.max(x1, c.x); y1 = Math.max(y1, c.y); };
  for (const gi of t.cells) add(gi); for (const p of office.peek) add(p.gi);
  const pad = 46; view.cx = (x0 + x1) / 2; view.cy = (y0 + y1) / 2;
...
function render() {
  ...
  const t = curThread(), TC = threadColor(t);
  ...
  // the current thread floor — ONE colour, rooms only slightly varied, hallway darker
  for (const gi of t.cells) {
    const onSpine = office.spine.has(gi), r = office.roomOf.get(gi);
    const fill = onSpine ? mix(DARK, TC, 0.14) : mix(DARK, TC, 0.30 + (r ? r.shade : 0));
    fillFloor(gi, rgba(fill, 0.98), rgba(mix(DARK, TC, 0.5), 0.35));
  }
  // muted role glyphs
  if (view.scale > 0.4) for (const r of office.rooms) { const p = P(r.cx, r.cy); ctx.fillStyle = rgba(mix(TC, INK, 0.6), 0.22); ctx.font = `${Math.max(8, 12 * view.scale) | 0}px "JetBrains Mono", monospace`; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText(r.glyph, p[0], p[1]); }
  // doors: a subtle gold seam at the threshold (the portal is hidden — you just walk through)
  for (const [gi, d] of t.doorAt) { const c = cells[gi], far = cells[d.farGi], mx = (c.x + far.x) / 2, my = (c.y + far.y) / 2, p = P(mx, my); const ncol = threadColor(threads.get(d.toKey)); ctx.beginPath(); ctx.arc(p[0], p[1], Math.max(2, 3 * view.scale), 0, 7); ctx.fillStyle = rgba(mix(GOLD, ncol, 0.4), 0.7); ctx.fill(); }
  // NPCs
...
// ── HUD ──
function updateHUD() {
  const t = curThread(), r = office.roomOf.get(state.gi), onSpine = office.spine.has(state.gi), atDoor = curThread().doorAt.get(state.gi);
