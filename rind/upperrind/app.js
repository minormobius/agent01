...
import { paintChunk } from '../ops/v101/skin.js';
import { drawWallFixture } from '../ops/v101/consoles.js';
import { drawDevice } from '../ops/v101/v5/deco.js';
import { drawWallLight } from '../ops/v101/v5/lights.js';
...
function updateHUD() {
  ...
  // the rail name wears the FLOOR hue (verb colour for whites, engine hue for production) so the
  // panel and the concourse underfoot read as the same flavour
...
updateHUD();
// the verb legend: the six whites in the order they ring the weave, each named by its dominant verb
{
  const el = $('legend');
  if (el) el.innerHTML = Array.from({ length: 6 }, (_, i) => {
    const v = dominantVerb(world, 'W' + i) || '—';
    return `<span class="v"><span class="sw" style="background:${rgba(floorHue(world, 'W' + i, TEAL), 1)}"></span>${v}</span>`;
  }).join('');
}
