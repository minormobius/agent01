...
- Constellation surface **verified live**: `GET blue.microcosm.links.getBacklinks?subject=<anchor URL>&source=com.minomobi.io.ticket:board` → `{ total, records:[{did,collection,rkey}], cursor }`. Two gotchas confirmed by probing (a) `app.bsky.feed.like:subject.uri` returned `total:4198` matching a known post's likes, and (b) our exact query returned `{"total":0,"records":[],"cursor":null}` (HTTP 200, empty — no tickets yet): the `source` path has **no leading dot** (`board`, not `.board` — a dot returns HTTP 400), the response key is `records` (objects with `did`/`collection`/`rkey`), and `cursor` is an opaque hex string. The `board` field is a plain URL string — Constellation indexes plain web URLs as targets.
...
- **"Tracked as…" reply bot is built** (§6, was roadmap). After the sweeper mints a ticket, `replyTracked()` posts an `app.bsky.feed.post` reply from the service account into the original thread (reply root/parent strongRefs + a UTF-8-byte-indexed link facet to `/board`). Best-effort — a reply failure never blocks the mint. Opt out with worker var `SWEEP_REPLY=off`. Like the rest of the sweeper, it's dormant until `ATPROTO_SERVICE_*` secrets are set.
...
only mutates D1 board state, never anyone's PDS). To re-lock them, set an
`IO_ADMIN_KEY` repo secret (deploy-io maps it → worker `ADMIN_KEY`); then the
routes require `X-Admin-Key`. The `SECRETS_PAT`-based provisioning workflow was
removed as unnecessary — if you ever want a key, set `IO_ADMIN_KEY` in the
GitHub UI and redeploy.
