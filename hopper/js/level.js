...
export function level(n) {
  ...
  return { n, seed: n, version: LEVEL_VERSION, slab, packs, off, climb };
}
...
export function bucketOf(lv, reach) {
  ...
  const z = Math.min(GRID - 10, top + Math.max(8, Math.round(rise * lv.climb)));
  const x = Math.max(8, Math.min(GRID - 9, lv.slab.x + lv.off.dx));
  const y = Math.max(8, Math.min(GRID - 9, lv.slab.y + lv.off.dy));
...
export function packLabel(p) {
  return `${p.habit} · ${p.masons} masons · ${p.budget.toLocaleString("en-US")} bricks · ${p.size}×${p.size} plate`;
}
