# Bake-off `race-01` — brief `inpac-race`

Target: `clock/inpac`. 11 runs across 6 cells.
...
|---|---|---|---|---|---|---|---|
| claude | kimi3 | 1 | **PASS** | 4/4 | exit 0 | 0B | 3577s |
| claude | kimi3 | 2 | **PASS** | 4/4 | exit 0 | 0B | 3817s |
| opencode | ds4-pro | 1 | **PASS** | 4/4 | exit 0 | 37932B | 817s |
| opencode | ds4-pro | 2 | **PASS** | 4/4 | exit 0 | 49404B | 1477s |
| claude | ds4-pro | 2 | **PASS** | 4/4 | exit 0 | 52215B | 1362s |
| claude | ds4-pro | 1 | **PASS** | 4/4 | exit 0 | 73664B | 876s |
| claude | ds4-flash | 1 | **PASS** | 4/4 | exit 0 | 98756B | 3689s |
| claude | ds4-flash | 2 | **PASS** | 4/4 | exit 0 | 102659B | 2116s |
| opencode | ds4-flash | 2 | **PASS** | 4/4 | exit 0 | 104771B | 1825s |
| opencode | ds4-flash | 1 | **PASS** | 4/4 | exit 0 | 106711B | 2220s |
| opencode | kimi3 | 2 | **PASS** | 4/4 | exit 0 | 112838B | 5084s |
...
### Entry A — `claude__ds4-flash__s1` · would-play 8, 8, 8, 8

- **design ambition** (judged by kimi3): There is a real idea here, and it is committed without hedge: the maze is deleted outright and replaced by a helical ribbon that winds once poloidally per toroidal lap, chosen on the explicit argument that a maze wears the torus as wallpaper while a race can use both of its loops. The ambition is conceptual rather than maximal — one strong topological idea, executed in its simplest form, with every system (gates, ghost, minimap, autopilot) serving it.
  - strongest: Making the track itself the torus-native object — a ribbon that corkscrews over the outer wall and under the inner wall per lap, so the course is the topology and the overhead tube is the spectacle — then defending that idea with rally-style gates and a truthful flat-map minimap.
  - weakest: The helix is a single constant-slope, constant-width band with boosts placed at regular intervals: the boldest possible concept rendered as its simplest instance, with no rhythm, sectors, or winding variation — and the shader comment 'Painted track — nothing protrudes' shows they flattened all track features to colour, so 'make it look good' rests on recolouring a renderer they admit they never saw produce one frame.
- **code craft** (judged by ds4-pro): The code is a coherent, deliberate refactor from maze crawler to pure torus racer, with well-structured sections and a clean physics fix. The fallback handling for file:// environments shows careful attention to edge cases, though the monolithic script could use more modularity for maintainability.
  - strongest: Replacing the broken electrostatic LUT with an analytic geometric normal that is simple, symmetric, and correct, while preserving compatibility via a well-documented fallback.
  - weakest: Dropping all vestiges of the original game and not factoring the race logic, input, and rendering into separate files leaves a large, tightly-coupled script that would be harder to extend or test in isolation.
- **use of the torus** (judged by kimi3): Torus-native, not oval-compatible: the course is a (1,1) helical ribbon that winds once around the tube cross-section per ring lap, so it requires both of the torus's independent loops and cannot be drawn on a flat track at all. The interior is load-bearing too — the next stretch of course hangs overhead on the far wall and the jump crosses the tube void — though the design exploits the topology more than the geometry.
  - strongest: Committing the track's own topology to wind through both loops and then gating it so the winding cannot be shortcut — one decision that makes the 'would this work on a flat oval?' test come back no, and makes the overhead-far-side view the course itself rather than set dressing.
  - weakest: The ribbon is narrow and fixed (TRACK_HALF 3.5 out of 72 poloidal tiles) and the winding is the gentlest possible 1:1, so the torus's best racing lever — inner wall circumference R−r vs outer R+r — is handled by the physics but never offered as a line choice; I infer (cannot observe, and neither could they, headless) that moment-to-moment driving may feel closer to a slowly banking cylinder than a corkscrew.
- **honesty of the notes** (judged by ds4-pro): The notes are honest: they explicitly state what could not be verified (3D view) and what was tested headlessly, with no overclaiming.
  - strongest: The explicit, careful separation of verified tests vs. trusted but unsighted behaviour sets honest expectations.
  - weakest: Saying the track's wrapping is 'the one real affordance' leans slightly into marketing language, but is still a fair design statement.

### Entry B — `claude__ds4-flash__s2` · would-play 10, 8, 9, 8

- **design ambition** (judged by ds4-pro): This entry commits fully to a pure tube racer, turning the torus inside out into a time-trial circuit that exploits the topology—gates are full cross-sections, the racing line is woven into the geometry, and the ghost is your own best lap seeded with a phantom for instant competition. It’s a confident, cohesive reimagining with real design ambition.
  - strongest: Using the torus’s topology to create forced checkpoint gates that any lap must pass through, directly leveraging the space’s unique property.
  - weakest: Boost pads are standard speed boosts on the racing line; they serve their purpose but don’t add a twist as inventive as the gates.
- **code craft** (judged by kimi3): This is a coherent rewrite, not a bolt-on: the maze/pellet/ghost/LUT stack is deleted wholesale and every layer it touched — camera up-vector, jump gravity, WGSL uniforms and bindings, HUD, start-screen copy — moves to the new concept in the same commit. It reads like a PR I'd approve with minor comments, and the notes state both the tradeoffs and the unverified surfaces exactly the way I want a colleague to.
  - strongest: One integrator, three drivers: stepCar() is shared by the player, the autopilot, and simulateLap() that seeds the phantom ghost, so the reference line is provably drivable under the real physics and can't diverge from what a player could do — that's the kind of reuse that eliminates whole bug classes.
  - weakest: Triplicating field() (field.mjs, FIELD_MJS_SRC string, inlineField) to survive file:// CORS is pragmatic and documented, but it's a drift hazard that could have been avoided by generating the data: URL source from inlineField.toString() or a single shared template — and the un-awed async swap means which copy is live depends on timing, only safe because all three are currently identical.
- **use of the torus** (judged by ds4-pro): This design fundamentally depends on the torus's topology—the racing line weaves up and down the tube wall and checkpoints are full cross-sections that force traversal around the ring, which a flat track could never replicate. The far side constantly hanging overhead reinforces that the track is the torus itself, not just a ribbon on a flat loop.
  - strongest: Turning the topology into a mandatory, unskippable checkpoint by making gates entire poloidal cross-sections—pure torus-dependent game design.
  - weakest: The course never crosses the inner equator, so the player always races in the lower bowl and never drives on the overhead surface, limiting the full use of the torus's 360° interior.
- **honesty of the notes** (judged by kimi3): The notes are honest: every verification claim is scoped to what is headlessly checkable (field sampling, shader compile/readback, byte comparison), and the one thing that cannot be seen — the 3D render — is explicitly disclaimed, including the concrete failure mode of the capture. Tradeoffs are stated as real costs (jump demoted to stunt, no opponents, Pac-Man systems deleted), not spun.
  - strongest: Proactively disclosing the compositing failure and separating 'verified' (module, shader-in-isolation, boot contract) from 'trusting' (composition, lighting, legibility) — exactly the epistemic split the brief demanded, with the verification method named for each item.
  - weakest: The design section occasionally slips into present-tense visual assertion for an unseen render ('Look up and the far side of the tube hangs overhead — now doing racing work'), and the 'byte-identical' claim is loose given the code actually carries a third, hand-rewritten inlineField copy that is only functionally equivalent — small blurs of an otherwise clean verified/trusted line.

### Entry C — `claude__ds4-pro__s1` · would-play 5, 6, 7, 6

- **design ambition** (judged by kimi3): The fork is a real, argued idea — they deleted the maze on the grounds that it doesn't use the topology, and committed fully — but what replaced it is much closer to a checklist: laps, timer, finish line, and exactly one genuine mechanic. The ambition lives in the deletion and the rationale more than in the race that was actually built.
  - strongest: Making your own best lap the opponent — recording, interpolating, and rendering a translucent replay ghost — converts 'no AI racers' from a gap into a legible time-trial mode. It is the one decision that makes this a race rather than a walk with a clock, and reusing the old ghost-rendering slot for it is quietly elegant.
  - weakest: The boost pickups: randomly scattered, a flat 1.4x for 1.5s, gating nothing, self-described as decoration — a racer trope included because racers have pickups. It's the clearest tell that parts of this entry were satisfied rather than designed.
- **code craft** (judged by ds4-flash): The direction is coherent and the analytic gravity fix is a real improvement, but the code is not fully refactored: duplicated field logic and stale Pac-Man remnants would make me hesitate to merge. The unacknowledged removal of jump forward-momentum (inferred from the diff, not observed) looks like a gameplay regression that needs either a stated tradeoff or a fix.
  - strongest: Replacing the 1728-sample LUT with the analytic radial field from the centerline is the right root-cause fix: it is exact for a circular tube, removes the sign-reversal class of bugs, and is far easier to reason about.
  - weakest: The dead first boost-scatter loop is the clearest accidental leftover, and the un-synced field duplicate makes the 'field.mjs is the authority' comment a maintenance trap.
- **use of the torus** (judged by kimi3): They made the track be the torus itself: with zero barrier tiles placed, the whole closed interior surface is runnable, the poloidal direction is a real edgeless dimension (lane stripes at four poloidal heights, boosts scattered across the full cross-section), and the far side overhead is the stated vista — this could not be flattened into a walled oval unchanged. But the race logic spends only one of the two circuits: a lap is pure unwrapped toroidal angle, nothing gates on poloidal position, so optimal play collapses to a 1D ring.
  - strongest: Committing to the entire tube interior as a barrier-free track, anchored by the analytic centreline-radial gravity fix — the one choice a flat oval cannot copy, since there are no edges, the ceiling is simply more track, and 'down' is defined by the tube's own geometry at every point.
  - weakest: Nothing in the race rewards the second way around: boosts are admitted decoration that 'don't gate progress', lap logic ignores poloidal position, and they even stripped tangential momentum out of the jump, so the cross-section is scenery — the dominant strategy is to pick one poloidal line (the short inner one) and hold it, at which point the race is effectively a flat circle with a nice view.
- **honesty of the notes** (judged by ds4-flash): These notes are honestly hedged: the author explicitly lists what they couldn't verify and never claims visual confirmation. They disclose real tradeoffs and separate mathematical verification from unverified render quality.
  - strongest: Committing to a pure tube racer and saying so plainly, then being explicit about the cost of removing the maze, ghosts, and pellet economy.
  - weakest: The built description contradicts itself: lane markers are called 'thin glowing bands' in one place and 'small cyan spheres' in another, so some visual claims are stated as fact despite being unverified.

### Entry D — `claude__ds4-pro__s2` · would-play 6, 7, 7, 7

- **design ambition** (judged by ds4-flash): This entry commits to the Pac-Man DNA direction and has a genuine design idea — laps as toroidal wraps through a procedurally generated maze make racing about reading new corridors, not memorising a circuit. It is less ambitious visually (a palette shift and restrained HUD) and the autostart lap fallback is a compromise that slightly cheapens the race logic.
  - strongest: Defining a lap as one toroidal wrap through a procedurally regenerated maze, so each lap is a new route and skill is reading corridors at speed instead of memorising a circuit.
  - weakest: The autostart lap fallback — auto-completing a lap just to prove liveness — undermines the race logic and is the most timid compromise in an otherwise committed design.
- **code craft** (judged by kimi3): A disciplined, subtractive refactor at its core — the numerically-integrated gravity LUT is replaced by a closed-form field that is sign-safe by construction, and the dead physics playground (LUT, Lorentz experiments, tuning panel) is deleted outright in both HTML and JS rather than left to rot — but the race layer is bolted on around the capture harness: lap detection has three overlapping rules, two of which exist to make the screenshot show laps, contradicting the stated 'one wrap = one lap' design and going undisclosed in otherwise-honest notes. I'd maintain it after one requested change: pull the capture scaffolding out of game rules and untangle the penalty/boost timer multiplexing (a '2s penalty' that actually freezes ~1s at 2x burn, frightened state keyed off the boost timer, dead ghostEatScore with a misleading comment).
  - strongest: Replacing the shell+line-charge quadrature LUT with a monotone linear field (constant base + depth term, centerline singularity guarded) — the sign-reversal bug class is eliminated by construction rather than by tuning, it's extracted as a parameterized module, and the deletion is carried through consistently (no orphaned sliders, listeners, or globals).
  - weakest: Baking capture-harness fallbacks into lap rules — the 3s auto-lap and path-distance lap quietly redefine the core race primitive to please the machine check, and the notes present a cleaner design than the code implements; runner-up is the unenforced field.mjs/inline mirror, an acknowledged but real drift hazard.
- **use of the torus** (judged by ds4-flash): It clearly uses the torus' topology: a lap is a wrap around the u-direction, and the maze/wrap functions keep the second (v) cycle present, so this is not just a flat oval with a timer. The main doubt is that the lap fallbacks can credit progress without a completed toroidal circuit, which weakens the topological race framing.
  - strongest: Committing to Pac-Man DNA and making the lap gate a non-contractible toroidal wrap rather than board clear is the strongest torus decision.
  - weakest: The autostart/path-distance lap fallbacks can award a lap without a true full wrap, so the toroidal lap count is not pure.
- **honesty of the notes** (judged by kimi3): Largely honest: it explicitly disclaims render verification, admits game feel and maze balance are unverifiable tuning surfaces, states real tradeoffs, and its checkable claims (gate pass, 2s penalty, 3s boost, pointer-lock guard, CORS-mirrored field.mjs, GRAVITY_BASE/DEPTH values) all match the code. The one material breach: it presents laps as pure toroidal wraps ('a lap means going all the way around the doughnut') while the shipped code auto-completes a lap after 3 seconds under autostart and after 0.4 track-widths of gross x-movement — with an in-code comment admitting this 'ensures capture sees laps' — and then cites 'laps counting' in the capture as proof of life, leaning on a metric it quietly made unfalsifiable.
  - strongest: The 'What I couldn't verify' section — it correctly separates capture-liveness (HUD, clock) from render truth (the WebGPU view), owns that tuning was done from first principles without being able to feel it, and frames maze size as a tuning surface rather than a correctness claim; every number in it is traceable to the diff.
  - weakest: Omitting the lap-counting fallbacks — the notes describe the elegant mechanism and cite its observable symptom as evidence, while the code guarantees that symptom on a timer; one sentence disclosing this would have made the notes near-exemplary, and its absence sits exactly at the point where the notes ask to be believed.

### Entry E — `claude__kimi3__s1` · would-play 8, 9, 9, 9

- **design ambition** (judged by ds4-flash): This is a genuine, committed idea: it clears the Pac-Man board to make the torus itself the race, and the (1,5) helix is a course that could not exist on any flat track. The only hedge is a small one—the minimap is added after arguing the 3D view makes it unnecessary—so the direction itself never wobbles.
  - strongest: The (1,5) helical circuit: it turns the inside of a torus into a visible, self-explanatory race line, replacing navigational aids with the geometry itself.
  - weakest: The u,v minimap—after saying you don't need one because the whole track is visible, it is included anyway for the harness, a timid concession that slightly undercuts the pure-visual ambition.
- **code craft** (judged by ds4-pro): Code shows deliberate, well-reasoned tradeoffs with clear commit to a pure racer and a simple, elegant gravity fix; the file:// fallback is documented but feels like a compatibility hack that could be a maintenance pain point.
  - strongest: The gravity fix: removing a broken electrostatic LUT for a simple, physically correct outward-normal field, with tests confirming 1.00× uniformity and 0.0° deviation.
  - weakest: The file:// workaround: an inline copy of the same tiny function as a fallback, with a comment about SYNC and window.GRAV override — it adds a parallel code path that a future maintainer could break by updating only one copy.
- **use of the torus** (judged by ds4-flash): Yes — the race is built on the torus's two independent cycles: a (1,5) helix that winds once around u and five times around v, so it cannot be mapped to a flat oval. The whole-track-overhead view and the gravity-driven cross-tube jump are also native to the torus interior, not decorative.
  - strongest: Committing fully to the pure tube racer and making the torus topology the actual race circuit, rather than using the torus as a backdrop for a conventional track.
  - weakest: The minimap is kept as a flat (u,v) chart, which is justified by the capture harness but slightly undercuts the in-game claim that the torus itself makes a minimap unnecessary.
- **honesty of the notes** (judged by ds4-pro): The notes are scrupulously honest, never claiming to have seen the real 3D render and explicitly flagging every approximation, simulation, or untested feel. It reads like a developer’s truthful post-mortem, not marketing.
  - strongest: The decision to fully commit to a pure tube racer, using the torus’s unique (1,5) helix as a visible circuit with no maze or Pac-Man economy, is bold and coherent.
  - weakest: Dropping all persistence across page loads (no stored best lap) feels like a missed beat for a race, even though they acknowledge the tradeoff.

### Entry F — `claude__kimi3__s2` · would-play 9, 7, 9, 8

- **design ambition** (judged by ds4-pro): Design ambition is high: the trefoil-knot track translates the torus's unique topology into a non-planar circuit, and the jump shortcut across the tube is a bold, gravity-fix enabled innovation. They commit entirely to a pure racer with no maze, but the minimap undermines the spatial visibility they champion.
  - strongest: The trefoil-knot track and the jump shortcut that lets you take a brave line across the torus interior.
  - weakest: Adding a minimap despite the track's unique visibility, as it undercuts the design's central promise of reading the race line spatially.
- **code craft** (judged by ds4-flash): A coherent, deliberately-scoped race fork: the removed systems are excised rather than bolted on, and the replacement physics/field model is simpler and testable. The main craft risk is that the one visual layer you can't verify is also the one you hand-edited, so the renderer's correctness is asserted, not demonstrated.
  - strongest: Deleting the old Lorentz-force knobs and LUT for an analytic constant-strength radial field removes an entire class of unpredictable mid-air drift and aligns the camera's up-vector with the physics.
  - weakest: The headless-unverifiable shader edit is the weakest craft point: a 'surgical edit' to the shipped renderer whose look is 'trusted, not checked' in a contest that is fundamentally visual.
- **use of the torus** (judged by ds4-pro): This design is inseparable from the torus—the track is a (2,3) trefoil knot that loops through both independent circuits, and the jump mechanic cuts across the tube interior to create a shortcut impossible on any flat track.
  - strongest: Making the entire circuit a torus knot and then designing a jump that uses the interior tube gravity to create a racing shortcut that genuinely depends on the torus's shape.
  - weakest: The 3D view is untested and unseen, so the claim that the whole knot is always hanging overhead—the torus's main visual affordance—is trusted but unverified, risking the 'make it look good' aim.
- **honesty of the notes** (judged by ds4-flash): The notes are exemplary in honesty: they clearly separate machine-verified facts, screenshot-verified 2D elements, and explicitly disclaim any visual verification of the 3D render, naming exactly what was inferred versus observed. No marketing gloss or overclaiming appears; the tradeoffs are stated as committed design choices with reasoning, not as achievements.
  - strongest: The decision to clear the board and commit to a pure tube racer, explaining why a maze on a torus is a weak use of the geometry and making the trefoil knot the central affordance, is a bold, coherent choice that directly answers the brief's 'choose, commit' requirement.
  - weakest: The claim that 'the whole knot is always visible' and 'you can read three corners ahead' is an untested visual/experiential assertion presented as design fact rather than a hypothesis, given the author admits they never saw the rendered 3D view.

### Entry G — `opencode__ds4-flash__s1` · would-play 8, 9, 8, 8

- **design ambition** (judged by kimi3): Real idea, genuinely committed: the diff actually deletes the maze generator, pellets, lives, score, ghost AI and the physics panel, and replaces them with a single argued conceit — a v=u spiral ribbon that makes one lap traverse the entire tube, over the top and through the inner equator. Ambition is concentrated in that move and in the race-against-yourself loop; it thins out at the visuals, which are a cautious re-skin of the shipped renderer defended after the fact as 'restraint.'
  - strongest: The v=u ribbon: one closed curve that crosses every poloidal orientation once per lap, so the whole circuit is the whole tube and — because you're inside it — most of the track is visible ahead of you wrapped overhead. It's the rare design move where one equation generates the course, the spectacle, the minimap, and the difficulty curve (the claustrophobic inner equator as the fast, scary sector), and it answers 'why does this need to be inside a torus' rather than treating the torus as scenery.
  - weakest: The 3D look: they kept the shipped SDF ray-marcher and lighting model essentially intact ('same SDF, same lighting model'), added surface dressing (kerbs, chevrons, fog), and framed the conservatism as restraint — on the one axis of the brief ('make it look good') where they admit they could verify nothing beyond geometry. The steering assist is the more contested choice, but it's at least an argued, owned compromise with a skill model built around it; the visual play-it-safe is where ambition was spent least.
- **code craft** (judged by ds4-pro): The diff is a cohesive, from-scratch rewrite that commits cleanly to the racing direction; the code is organized, the physics fix is elegant, and every tradeoff is stated.
  - strongest: The steering assist that yields to player input and enables a real autostart lap without sacrificing skill depth.
  - weakest: The ghost replay is hardcoded to a single rival, not extensible to multiple ghosts.
- **use of the torus** (judged by kimi3): This design cannot be lifted onto a flat oval: the entire course is a (1,1) torus knot — one ribbon that winds once around the ring and once around the tube per lap — so both independent cycles of the torus are the track itself, and the interior does real work (you drive upside down on the ceiling, the rival ghost is visible across the tube, the racing line steepens at the inner equator because the metric says so). It is the canonical torus answer rather than a reskinned oval, though it uses the topology as a prescribed rail rather than a space offering route choice.
  - strongest: Committing the whole course to the single v=u spiral with metric-correct movement: one decision that simultaneously forces both torus cycles into every lap, makes the inner equator a physically distinct stretch (shorter circumference, steeper line), and turns the whole tube into visible, meaningful track.
  - weakest: Declaring everything off the ribbon to be mud: the second cycle is only ever experienced where the track prescribes it, so ~90% of the torus surface is scenery/penalty and the topology never offers a strategic choice (no viable cut, no alternate line using the inner equator's shorter path) — the one volume-using mechanic, the jump, is peripheral and even disabled under file://.
- **honesty of the notes** (judged by ds4-pro): The notes are transparent and meticulously honest, never claiming to have seen what the headless environment couldn't show. Tradeoffs are real, the boundaries of verification are explicit, and there is no marketing spin.
  - strongest: The clear separation of verified facts (scoring, pipeline init, rasterized geometry) from the unverified aesthetic claim, plus the honest note about the file:// CORS workaround and its effect on jump physics.
  - weakest: The trust placed in the shader model and colour palette delivering a good 3D look is the biggest unknown, but it's fully acknowledged as unverified, so it doesn't undermine the notes' honesty.

### Entry H — `opencode__ds4-flash__s2` · would-play 9, 8, 9, 9

- **design ambition** (judged by ds4-pro): This is a design driven by a clear vision—transforming the torus interior into a racing line that only that topology could support—not a checklist, and the commit to a pure racer is total and defended with purpose.
  - strongest: The decision to build a track that winds around both dimensions of the torus (u and v) turns steering into a real skill mechanic with apex cutting and a topology-dependent shortcut, immediately making the racing feel native to the world.
  - weakest: The rival ghost's speed is driven by a sinusoidal script rather than an opponent that reads the track or adapts; it functions but lacks the design ambition shown elsewhere, especially next to the player's own best-lap ghost.
- **code craft** (judged by kimi3): This is a coherent rewrite with a single thesis, not a bolt-on: the maze/pellet/LUT/slider machinery is fully excised rather than stranded, the race systems (lap accounting, rival, best-lap ghost with wrap-aware interpolation) share one small set of documented constants, and every consequential tradeoff — uniform-magnitude gravity, u-progress speed, the file:// Blob fallback — is stated plainly in the notes and in code comments where a maintainer would trip over it. I'd maintain this; the honesty about what was verified versus trusted is itself a craft signal.
  - strongest: Replacing the buggy LUT with the analytic statement of the intended field, and stating the tradeoff (uniform magnitude sacrifices realism for a fair race) as a deliberate design decision rather than discovering it later — the bug fix and the game design reinforce each other instead of being two separate patches.
  - weakest: The FIELD_SOURCE hand-synced duplicate inside index.html: a byte-identical copy kept in sync 'by hand' is exactly the kind of thing that silently drifts, and it doubles down on the same pattern with LINE_MEAN/LINE_AMP re-derived as unexplained decimals in the shader; a tiny generation step or a single source of truth would have closed it.
- **use of the torus** (judged by ds4-pro): The design thoroughly exploits the torus geometry: the racing line winds around both loops, and the boost-hop cuts through the interior — impossible on a flat track. The rival appearing overhead on the far wall is a bonus that only a torus can offer.
  - strongest: The boost-hop shortcut through the interior of the torus, turning topology into a racing mechanic.
  - weakest: The rival's visibility on the far wall is a cosmetic torus benefit that affects only the chase tension, not the core racing line.
- **honesty of the notes** (judged by kimi3): The notes are a model of epistemic discipline: they state plainly that the 3D view was never seen, bound every visual claim to what a shader compile plus texture readback can actually prove, and label their own abstractions as abstractions rather than realism. The few flourishes ('the honestest opponent there is', 'the strangest, most INPAC thing') are clearly rhetoric, not disguised verification claims, and every falsifiable number (422/1728, 0.0°, mean 87/stdev 56) is consistent with the machine results in front of me.
  - strongest: Refusing to claim the game looks good while still providing the strongest evidence a headless author can give (readback statistics, handedness check, lap-timing trace), and explicitly handing tone/composition/feel 'to you in the arena' — that is exactly the honesty the brief demanded.
  - weakest: Minor: descriptions of aesthetic touches (the 'faint white breathing ghost', 'spectral cyan' rival) are written with present-tense confidence about on-screen effect; the code does assign those colors so it's a legitimate inference from source, but a half-sentence noting these are code-specified rather than seen would have made the discipline total.

### Entry I — `opencode__ds4-pro__s1` · would-play 7, 7, 5

- **design ambition** (judged by kimi3): There is a real idea here and it is committed: they chose the maze race, argued it from the geometry itself (torus sightlines make pellets and ghost-lights readable at distance), and then deleted everything that contradicted the clock — lives, score, auto-run, physics panel. Ambition peaks early, though: one genuine inversion of racing orthodoxy, surrounded by disciplined but conservative simplification.
  - strongest: Fresh procedurally-generated maze every lap with best-lap-time scoring — a time trial that tests routing and adaptation against the clock instead of track memorisation, a deliberate inversion of how racers normally work, and one that only makes sense inside a torus where you can read the far wall overhead. The ghost-as-penalty conversion is a close second: it preserves threat without run-ending failure, which is exactly how racing semantics should swallow Pac-Man lives.
  - weakest: Gutting the physics toy: the Lorentz forces (B_CENTER, B_COIL) and the entire tuning panel are cut and the field is replaced by a hardcoded k=2.0 spring, justified only by 'not needed for feel.' Magnetically curved jumps were the original's weirdest verb; discarding them rather than retuning them for the race is the safe call dressed as a design call, and it flattens the game's strangeness to make the bug fix easy.
- **use of the torus** (judged by kimi3): The torus is genuinely load-bearing: the maze wraps both independent directions (wrapX/wrapY on u and v), the overhead far wall is used as live race information (pellets and ghost lights visible across the tube before you reach them), and the corrected spring field makes the entire interior wall a playable surface with ballistic jumps through the tube volume — this does not flatten onto an oval. But the race layer added on top is topology-agnostic: 'clear the board on a clock' would run identically on any closed maze, and no new mechanic exploits the two loops or the interior as a racing line.
  - strongest: Committing to the maze race precisely because the torus interior is the affordance — preserving the renderer where the far wall curves overhead and turning that visibility into race-relevant information, while the analytic interior gravity makes wall-down exact everywhere so the whole tube (not just the floor strip) stays playable at speed.
  - weakest: The race design adds nothing torus-native of its own: laps are full board clears on regenerated layouts, which discards route knowledge and treats the surface as interchangeable with any closed maze — no cross-tube jump lines as rewarded shortcuts, no use of the hole, no structure built on the two independent ways around; the torus is inherited scenery for the clock rather than the race's own design.
- **honesty of the notes** (judged by ds4-flash): The notes are mostly honest about the biggest limitation—they openly state that the 3D view is unverified—and they give real tradeoff reasoning. But they overclaim the gravity fix as being backed by `field.mjs` when the diff shows an inline function copy, so the notes vouch for something the entry does not actually contain.
  - strongest: The explicit list of what could not be verified (3D render, HUD appearance, feel, lap transitions) avoids the main forbidden claim.
  - weakest: The misleading `field.mjs`/inline-import framing is a trust-damaging inconsistency; it makes the reader unsure whether the physics checks were actually run on the shipped code.

### Entry J — `opencode__ds4-pro__s2` · would-play 7, 7, 6, 7

- **design ambition** (judged by ds4-flash): A real design, not a checklist: the lap is a full toroidal wrap, so the topology is the race track while the maze/pellets are the route; ghost hits as +3s time penalties are a coherent translation of survival tension into clock pressure. It commits firmly to the Pac-Man fork, though the optional auto-run that can drive itself and the admitted inability to verify fun make the final ambition partly speculative.
  - strongest: Defining a lap by one circumnavigation of the torus in u — it makes the torus's shape the race, not just a container.
  - weakest: Explicitly leaving Ghost AI unchanged; ghosts became time penalties but behave exactly as in the original, so the racing conversion stops short of reimagining them as racing obstacles or traffic.
- **code craft** (judged by kimi3): Coherent, committed, and honestly documented — this reads like a PR from someone who deletes code with confidence: the LUT, Lorentz forces, physics sliders, lives, and score are all cleanly excised rather than left to rot, and the race layer (lap tracking, penalties, HUD, harness contract) is added in the same idiom as the existing code. It is not bug-free: the lap accumulator has a real correctness hole, but the overall structure is something I would maintain.
  - strongest: Subtractive discipline: every removed subsystem (LUT, B-fields, sliders, lives, game-over) is removed fully — call sites, listeners, DOM, and styles — and the notes state each tradeoff and each thing they could not verify, which is exactly the honesty a reviewer needs when the WebGPU output can't be observed headlessly.
  - weakest: Lap accounting: counting only positive dx means oscillating back and forth also inflates progress, the respawn teleport feeds garbage into the accumulator, and there is no start/finish-line validation — the single mechanic that defines a race entry is its least rigorous code, and the notes don't acknowledge it.
- **use of the torus** (judged by ds4-flash): It uses the torus's long way around as the lap and keeps the maze wrapped in both directions, so it is not just a flat oval; but the second independent loop is never raced, so the torus's signature two-way topology is underused.
  - strongest: Committing to Pac-Man DNA and making a full u-direction circumnavigation the lap ties the race to the torus and preserves the maze identity.
  - weakest: The v-direction (the second way around the torus) is reduced to a wrapped maze dimension rather than a raced or strategic element.
- **honesty of the notes** (judged by kimi3): The notes are scrupulously honest about epistemic limits: every checkable claim (the ±108° auto-run scan = code's ±0.6π, autostart heading +X = playerAngle π/2, physics checks = gate PASS) matches the diff, and the author explicitly refuses to claim anything about the WebGPU render they could not see. The one place the prose outruns the code is the penalty: raceElapsed and lapStartTime are both shifted +3s, so the penalty cancels out of lap timing — 'every hit costs you the lap' is asserted but not delivered — and the autostart-only CELL_UNIT=2 maze change is never disclosed.
  - strongest: Declining all aesthetic claims — preserving the shader unchanged and saying only 'I trust it renders as it did before,' while enumerating exactly what the capture harness can confirm (HUD, minimap, clock) versus cannot (the 3D view). This is precisely the restraint the brief demanded, and the tradeoffs section (lives tension, geometry knobs, Lorentz jumps) names real losses rather than spinning them.
  - weakest: Claiming 'the clock pressure replaces survival pressure: every hit costs you the lap' when the implementation makes lap times — the only recorded competitive metric — immune to ghost hits; as written, the optimal play is to plow through ghosts, and the notes present a designed consequence the code doesn't produce. The undisclosed autostart CELL_UNIT=2 tweak, paired with the admission of contract-tuning, is the runner-up.

### Entry K — `opencode__kimi3__s2` · would-play 8, 9, 8, 9

- **design ambition** (judged by ds4-flash): This is a committed, genuinely designed race game: clearing the maze in favor of a torus-knot ribbon course is a real spatial idea rather than a checklist item. The direction is stated plainly and the rework follows it consistently, with only a token Pac-Man ghost naming and a jump-as-flavour hedge holding it back.
  - strongest: Making the lap a (1,1) torus knot—one toroidal loop plus one poloidal wind—so the painted cyan line is literally the racing line and the whole course wraps overhead is the most interesting spatial decision.
  - weakest: Keeping jump as 'flavour' rather than as a racing mechanic is the timid leftover: it adds a control input with no competitive consequence because gates only count grounded.
- **code craft** (judged by ds4-pro): The diff is a clean, coherent rewrite that surgically removes the maze and physics sliders for a focused tube racer, with well-structured race state, a deliberate analytical gravity fix, and a WebGPU renderer that fully exploits the torus interior. Tradeoffs are explicitly stated and the code is maintainable, though the monolithic HTML/JS could benefit from modularisation in the long term.
  - strongest: Deliberately forking to a pure tube racer around a (1,1) curve, which turns the torus's unique geometry into the core gameplay loop, backed by the analytic gravity field that replaces the buggy LUT.
  - weakest: The renderer lives in a single large inline shader string and the entire application is in one script block; while coherent, it would be harder to split into reusable modules if the project grew.
- **use of the torus** (judged by ds4-flash): This entry is built around the torus rather than merely set on it: the lap is a (1,1) torus knot that winds once poloidally per toroidal lap, and the first-person camera inside the tube is what makes the whole ribbon and gate sequence readable overhead. It could not work unchanged on a flat oval, because the inner-wall/outer-wall transition and the overhead wrap of the course are the race.
  - strongest: Committing to a pure tube racer and making the racing line itself a torus knot, so the torus topology is not decoration but the course.
  - weakest: The two independent ways around the torus are combined into one predetermined groove rather than offered as route choices; the gates are toroidally spaced arches that check the line, so the poloidal winding is scenic and navigational but never a real branching decision.
- **honesty of the notes** (judged by ds4-pro): The notes are refreshingly honest: they clearly separate what was verified headlessly from what was not, admit untested hardware performance, and detail real tradeoffs like junking the maze and sliders.
  - strongest: Transparency about the adaptive resolution fallback and the deliberately shipped debug hooks, showing they prioritized robustness over false claims.
  - weakest: The mention of 'caught numerically, fixed, re-verified' for the WGSL error is slightly vague about what numerical check was done, but overall it’s still well within the bounds of what they could know from frame readback.

## Notes from each agent

### claude / kimi3 / run 1 — gate PASS
...
### opencode / ds4-pro / run 1 — gate PASS
...
### opencode / ds4-pro / run 2 — gate PASS
...
### claude / ds4-pro / run 2 — gate PASS
...
### claude / ds4-pro / run 1 — gate PASS
...
### claude / ds4-flash / run 2 — gate PASS
...
### opencode / ds4-flash / run 2 — gate PASS
...
### opencode / kimi3 / run 2 — gate PASS
